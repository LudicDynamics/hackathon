...
预期的 `requirements_not_met` 是场景内反馈：使用 Gate 的顶层 `blocked` 短旁白、缺失物的本地化标题与短暂描边，允许返回现场或 Escape 收起；不显示 HTTP/JSON，不触发作家或自动骰子。真正的网络、配置与服务故障仍明确报告。实现与迁移边界见 [体验版稳定路径与门禁演出](体验版稳定路径与门禁演出.md)。
