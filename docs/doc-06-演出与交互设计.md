...
### 1.7 结局门与结果 Folder：让完成感发生在世界里

最终条件满足时，不弹出脱离世界的“游戏结束”窗口。对应 Gate 从不可进入变为可进入，玩家完成一次明确的承诺动作后，镜头迁移到 Outcome Folder。进入顺序是：门槛状态变化 → 确认不可误触的行动 → 转场 / 声场收束 → 结算 Chalk → 角色与实体按结果落位 → 显示继续、下一章或回到检查点的世界内出口。

结果 Folder 必须至少留下一个持久事实，并让一个角色明确回应玩家造成的变化。重置只回到标明的篇章检查点，不默认清空玩家身份、跨篇章关系或世界历史。具体四世界语法见 `doc-20` §2.11–§2.15。
...
### 2.6 叙事 frontmatter 的渲染（choice / status / roll_dice）

**渲染方式：类 md 表格渲染，不是 HTML。** 三者都绑在 `type: chalk` 文本的 frontmatter 上，渲染在主板书正下方，可折叠。

```
---
type: chalk
roll_dice:          # 骰子卡：玩家点击掷出 → 演出特效 → 结果回写（doc-05 §3.1「骰子协议」）
  type: 1d100
  desc: 检定是否通过
...
- **choice**：渲染为**选项卡片组**（行动卡），排列在主板书正下方，点击 → 作为玩家下一轮输入推进；
- **status**：渲染为**折叠表格**（键值对），可展开/收起，随叙事走；
- **roll_dice**：渲染为**骰子卡**（doc-05 §3.1「骰子协议」）——作家只写 `type`/`desc`/`expect`（**不写 `result`**；`expect` = 引擎可读的通过条件比较式，如 `">50"`，**必须带引号**）；**玩家点击骰子** → 骰子演出特效（纸世界语汇：纸骰弹跳/旋转 → 落定）→ 服务端真随机掷出，**按 `expect` 判定** → 结果**回写 `result`** → 卡片落定显示（数值 + 通过/未通过）；结果同时**作为事件落事件表**（type: `roll_resolved`，doc-05 §5.1）供作家下一轮感知；
- 三者都可折叠——"状态/选项/骰子绑定在一段具体叙事文本中，前端天然可折叠"。
...
| **选项** | 作家可塞（choice frontmatter 迁移：选项可带 `until` 过期——过了时机不再可点），选项是快捷方式不是唯一通道 |
...
**角色是"拿到最近场景情况但独立于作家"的 agent**：

- **形态**：按需 spawn 的独立进程（懒灵魂——第一句话才启动），有自己的 preset/记忆/目录；
- **上下文**：preset 引用文件（identity/appearance/personality/memory）+ 最近场景情况（invoke 时注入，让角色知道此刻场景里发生了什么）+ 自己手动 read_canvas；
- **独立性**：角色按自己的记忆/人格回答，**作家不介入、不考虑角色意见**——作家写 chalk 板书叙事时不需要考虑角色想法；
- **生命周期**：进入遮罩 spawn，对话结束（Esc/说完）可回收；记忆写进角色自己的 `memory/`，跨遮罩会话连续；
- **顺手写**：遮罩对话中角色可顺手往自己根目录（`characters/<角色名>/`）写痕迹（玩家不可实时见，但提示"角色留下了东西"）。
...
5. 对话中途 Esc，角色记不记得你半途而废（角色记忆——架构层课题，原型不做）。
