...
niko 作家反馈：不投影每次 Chalk 工具调用的临时正文；每轮 `writer_idle` 后只显示最后一条公开 `writer_message` 的短预览（最多 240 字符），约 7 秒自然淡出。同屏最多一条，新轮开始或切场景即清除，不展示 reasoning/thinking 或工具参数。无公开回复时仅提示行动处理结束，不宣称已落板。真实 Markdown 板书仍保留在画布与文件系统中。
