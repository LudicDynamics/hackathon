...
6. `docs/前端改造计划.md` / `docs/后端实现计划.md`（施工单，**行号可能过期，实现前用代码复核**）。
...
- **必须排除 hover 态**（04 实测新坑）：chalk 的 inline status header 在 `pointerover` 展开，`offsetHeight` 578→664（+86px）。测量时 MUST NOT 在被 hover 放大的卡上取样。
...
今天 `reseatLayer:599-604` 把"行值 ≠ 表值"当**漂移信号**。一旦实测值写进列，**每张 chalk 都会被判为漂移并被冲回 `form.h`**——修复反而被打回。

**必须改为**（五个漂移源，按序短路）：

1. **行缺失** → 首次排座。
2. **`w/h <= 0`** → 脏行修复。
3. **`row.measuredAt` 非空 且 (`row.h !== meta.seatH \|\| row.w !== meta.seatW`)** → **实测占位变了**（本契约的 `seatH` 补链，§5.2）。这是**唯一**能让"卡长高了 → 座位让开"发生的路径；漏了它，F1 的目标不达成。
4. **`rowVersion` 存在 且 `!== declaredVersion`** → kind 在代码里被改过尺寸（`reseatLayer` 的**原始正当用途**，`local-store.ts:587-593`）。**`rowVersion` 缺失时不算失配**（走 legacy 分支，见 §5.2 末）——否则 `writeFootprints` 刚落 `measuredAt` 但还没 `formVersion` 的行会被误判重排，冲掉刚测到的高度。
