...
> **跨批次接缝（2026-09-13）**：footprint 只提供真实 `cards.width/height` 与四键 metadata；生成卡片的列流排版和 drift 重排统一见 [`docs/layout/00-共同上下文.md`](../layout/00-共同上下文.md)。layout 不得扩展本文件 metadata，也不得把 declared 高度当长期碰撞尺寸。
