...
> **UX 批次回写（2026-09-13）**：本文旧的 `writerToolsInFlight` 计数示例不再是 writer busy owner。调度器的 `isBusy()` 必须消费 `docs/ux/03` 的 canonical `WriterPublicState` 派生 `isFootprintBusy = phase === 'writing' || toolCount > 0`；断线 reset 由同一 writer adapter 完成，本文不得保留独立计数 ref。

---

## 1. 一句话定位

**把“作家不忙”定义为 UX03 projection 的 `isFootprintBusy === false`：** `useWorld` 在「层渲染完成 + 字体稳定（`whenFontsSettled`，契约 §3.5）+ 作家不忙 + 不在拖拽 + 不在 hover 展开态」时用一次 `offsetHeight` 全层测量，200ms 尾防抖后对 `POST /api/card/footprint` 发**一次**全层盒子；路由照抄 `POST /api/card/position` 的校验体例，**只**调 01 的 `store.writeFootprints(layer, boxes)` 并透传 `{ok, updated, unchanged}`；**不落事件、不广播帧、不回流**（不触发 `fetchLayer`）。
...
> 漏了会怎样（F3）：
> - 不设②：流式写作期间每出现一个 200ms 静默空隙就发一次 POST——**服务端被灌入一串中间态**；每次 POST 都写 canvas.db → 回流会触发一次受控的最新层读取，形成**活锁**。这是本设计里最易漏、后果最重的一条。
...
**Canvas SQLite 回流裁决（2026-09-14）**：`canvas.db` / `canvas.db-wal` / `canvas.db-shm` 只唤醒 history tail reader，不再广播 `file_changed`。画布坐标、线条和 footprint 各自已有显式通道；否则一次 footprint 写入会产生重复层读取与重复 reseat，玩家会看到卡片闪动。其余世界正文仍由 `file_changed` 驱动。
...
> 漏了会怎样：服务端的 SQLite 写入过滤是第一道闸，F6 是第二道闸。即使未来某个入口误触发重取，同层同指纹也不会再次 POST；两道都保留才能同时抵御 watcher 漂移和浏览器亚像素抖动。
...
sequenceDiagram
  ...
  Note over DB,WS: "canvas.db* 不广播 file_changed；仅唤醒 history tail reader<br/>画布状态由显式 frame 或 reconnect fetch 覆盖"
```
...
case 'tool_start':
  if (msg.source === 'writer') {
    writerState.acceptWriterFrame(msg); // UX03 updates canonical toolCount
  }
  break;
case 'tool_end':
  if (msg.source === 'writer') {
    writerState.acceptWriterFrame(msg);
    scheduler.notify();          // 稳定窗口已开始 → 武装一次测量
  }
  break;
```

- `tool_start`/`tool_end` 由 `event-bridge.ts:77/:108` 产出，**已存在**；writer busy 与 toolCount 由 UX03 canonical adapter 更新，本通道只在 `tool_end` 后 notify。
- 重连由 UX03 `resetForReconnect('socket_open'|'socket_close')` 归位；本文不维护计数 ref 或 `busySinceRef`。
...
### 6.3 回流（`file_changed`）——Canvas SQLite 不再触发

`POST` 写 `canvas.db` 后，`EventBridge.watchWorld` 仍会调用 `drainWorldEvents()`，但 `.airpworld/canvas.db`、`canvas.db-wal`、`canvas.db-shm` 不再广播 `file_changed`。坐标、线条和在场位置各自走显式帧；断线重连时由 `ws.onopen` 的 layer fetch 兜底。世界正文文件仍由 `file_changed` 驱动。

**结论**：footprint 写入不会再间接产生一次重复层重取，因此不会把同一尺寸变化重复送入 `reseatLayer`。F6 指纹闸仍保留，用来抵御浏览器亚像素抖动或未来误触发的重复测量。

- **事件可靠性边界**：过滤只影响瞬时 `file_changed`，不影响 history tail reader；显式 `card_position` / `canvas_patched` 帧和 reconnect fetch 覆盖画布当前值。
- **可见收益**：一次 footprint 更新最多触发调用方主动安排的一次最新读取，不再额外叠加 watcher debounce 产生的第二次读取。
...
**E-2（Canvas SQLite 不再广播 `file_changed`）**：浏览器实测一次 `/api/card/footprint` 写入后只出现该 POST 和一次 `/api/layer`（由 scheduler 的 `onResult` 主动触发），没有 watcher 追加的第二次 `/api/layer`。`history.db` 仍由 watcher 唤醒 tail reader。
...
1. **Canvas SQLite 回流已裁决并落地（2026-09-14）**：`event-bridge.ts` 对 `.airpworld/canvas.db*` 只保留 `drainWorldEvents()` 唤醒，不再广播 `file_changed`；`shouldBroadcastFileChanged` 回归测试和浏览器网络计数已通过。
