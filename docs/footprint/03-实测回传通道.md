...
| `routes/world.ts` 的 `cardSize`（`:81-84`） | 读 `cardFormOf` → `form.w/h`，喂 `seatUnplaced`/`reseatLayer` | 归 02：**拆成两个 helper**——`declaredSizeOf(item)`（喂 `reseatLayer`，返回 `form.w/h`）与 `storedSizeOf(item, row)`（喂 `seatUnplaced`，返回 `row.w/h ?? form.w/h`），契约 §3.4 冻结。**本文不改**，但在跨模块说明里点它（§11 冲突 1） |
