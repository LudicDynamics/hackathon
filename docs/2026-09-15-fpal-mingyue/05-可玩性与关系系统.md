...
> 批次：`docs/2026-09-15-fpal-mingyue/`。本文遵守 `00-共同上下文.md` 冻结契约，**不改动它**；冲突写进 §11。
...
**`templates/fpal` 今天不能玩，根因不是内容不够，而是它一个 `kind: 'character'` 都没有——「探索 → 互动 → 回应 → 关系变化」这半圈里，后半圈的三跳全部断线。** 本文把这条链路逐跳接死（声明 → 校验 → WS/HTTP → 前端对话），并回答「对话结束后关系怎么体现」——**在不新增任何状态命名空间的前提下**，用**世界事件**作玩家可见载体、**角色记忆库**作角色内心载体，两条路各自写什么写死在 §4 的对照表里。
...
| # | 跳 | `file:line` | 漏了会怎样 |
|---|---|---|---|
| 1 | 卡面渲染 choice 按钮组 | `EntityInteractions.tsx:302`（`renderFrontmatterWidgets`，门卡与普通卡走同一套） | 玩家看不到按钮 → 声明等于不存在（这是今天 fpal 的状态） |
...
工作树实测：`templates/fpal/world` 全目录**零个**物件把「小苍兰/白兰花」写进自己的正文（`grep -rn "小苍兰\|freesia\|白兰" templates/fpal/world` 只命中 `unit-602/bedroom/README.md:38`、`unit-602/balcony/washing-machine.md:19`、`unit-602/balcony/README.md:38` 三处**洗衣液香味**的描写——那是玩家家的气味源，不是「含义」）。玩家**没有**任何可交互对象叫「小苍兰」。→ **没有落点，就没有解释可以写进去。**
...
1. **解释权永远不开放**：即使玩家主动问明月「你知道那是什么味道吗」，`fpal-play/SKILL.md §知识边界:20` 的规则（「角色只知道他亲眼见过、或被人告知过的事」）在她身上的落地是——**她见过的是「1806 窗台上摆过一盆白兰花，后来收起来了」**（契约 §3.1 的关键观察），**她不知道那意味着什么**。这是**事实与解释的分离**：她**可以**说出她看见的事实，**不可以**说出含义。
...
**这本身是一条设计结论**：`kind:'character'` 的机制**已经完整存在**（22 个模板在用，见 §9 D-1）。本批要补的**不是引擎能力，而是 fpal 世界对它的使用**——这正是「99 个物件 / 0 个角色动作」这个诊断的意义。

---

## 9. 与现状的差异（「现状是 X」必带 `file:line`）

### D-1：fpal 有 **0** 个 `kind: 'character'`（根因）

```
$ grep -rn "kind: character" templates/fpal | wc -l
0
$ grep -rho 'kind: [a-z]*' templates/fpal/world --include=*.md | sort | uniq -c
     76 kind: enter
     60 kind: read
      5 kind: writer
```

- **99 个 md**（`find templates/fpal/world -name '*.md' | wc -l` = 99）、**33 个层**（`find -type d | wc -l`）、**33 个文件含 `choice_actions`**、**141 行 `kind:`**。
...
> **⚠️ 过时括注（2026-09-15 落盘后）**：本节是**本批开工时**的根因诊断。落盘后 `kind: character` 已有 **5 处**（本批由 04 补），D-1 的「0 个」不再成立；下方计数（99 个 md / 33 层 / 141 行 `kind:`）保留为当日实测——今天的实测是 `templates/fpal/world` **142** 个 md、`kind:` **163** 行。
...
### D-3：fpal 有 **3 个死场景**（只有 README，却带「做什么」选项）

工作树实测：**`templates/fpal/world` 下「直接子级 md 数 = 1 且存在 README」的目录共 13 个**；对每个目录解析本层 README 的 `choice_actions`，**恰好 3 个含非 `enter` 动作**：

```
templates/fpal/world/stellar-cloud-campus/tower-bc/joint-innovation-lab  {action-1: writer, action-2: enter}
templates/fpal/world/stellar-cloud-campus/riverside-park                 {action-1: writer, action-2: writer, action-3: enter}
templates/fpal/world/stellar-cloud-campus/sunken-plaza                   {action-1: writer, action-2: writer, action-3: enter}
```
...
> **⚠️ 已修（2026-09-15 落盘后，主 agent 复核）**：本节是**本批开工时**的实测，**三个死场景已按本节的处置被 04 修掉**——`joint-innovation-lab` / `riverside-park` / `sunken-plaza` 各新增一份 `type: chalk` 的 **`00-hub.md` 场内实体**（承接 `writer` 选项），三份 `README.md` 里**已无 `choice_actions`**。⇒ 本节的脚本判据（「直接子级 md 数 = 1 且 README 含非 `enter` 动作」）**今天实测命中 0 个**，与主 agent 落批后的扫描一致（同时「直接子级 md 数」也因新增 `00-hub.md` 变为 2，判据本身也随之不再适用）。

> 更正说明：本文初稿据一次手写 grep 误判 `joint-innovation-lab` 为「纯门牌」——其 `action-1` 实为 `kind: writer`。上表由脚本重扫全 fpal 得出，三个死场景与主 agent 独立体检结果一致。

### D-4：`CharacterModal` 已完整实现，但 fpal 从未 spawn 过明月
...
- **没有这个改动就会失败**：修前该命令返回 **0**（实测），因为 fpal 一个 `character` 动作都没有 → 玩家永远无法打开明月的对话 → 「互动 → 回应 → 关系变化」半圈**完全没有起点**。这是 §9 D-1 的直接推论。
...
grep -rniE "小苍兰|freesia|白兰|magnolia|卢卡斯|lucas" templates/fpal/world | grep -vE "洗衣液|洗衣|香型" 
```
...
- 另断言：`templates/fpal/characters/mingyue/preset.json` 的 `tools.deny` MUST 含 `get_state` 与 `state_update`（`elias/preset.json` 与 `presets/character.json:28-29` 是范本）。
