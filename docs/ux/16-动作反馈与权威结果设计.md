...
**已登记的 direct-API exception（2026-09-15，niko）**：带 `choice_actions` 的选项（`docs/settings/00`「显式直接动作」：`enter` / `read` / `take` / `character` / `stage`）**点击即经 coordinator 调 `/api/choice`**（`EntityInteractions.tsx` 的 `choose` → `executeDeclaredChoice`），由权威返回的 `action` 决定进层、打开阅读 / 拿取 / 材料对话框或角色。理由：这些动作由 HTTP 组合既有动作、不经作家，把它们改成草稿会让「回到工作室」也要等作家跑一轮。`kind: writer` 的声明仍只填草稿（`handleDeclaredAction`，见 `docs/security/01`）；没有 `choice_actions` 的普通选项仍只填草稿。守卫：`apps/web/test/declared-action-web.test.mjs`「declared choices run on click」。
