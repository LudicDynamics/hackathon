...
**已登记的 direct-API exception（2026-09-15，niko）**：**所有选项点击都直接调 `/api/choice`**，不进 writer draft——这是 `docs/settings/00` 的口径：选项点击 = 落一条 `choice_selected` 事件，作家是否接手由每世界 `autoWrite` 档位决定（`scenes-and-choices` 才起一轮）。
- `EntityInteractions.tsx` 的 `choose` → `executeDeclaredChoice`：经 coordinator 调 `/api/choice`；带 `choice_actions` 的选项（`enter` / `read` / `take` / `character` / `stage`）由权威返回的 `action` 决定进层、打开阅读 / 拿取 / 材料对话框或角色；`kind: writer` 的声明仍只填草稿（`handleDeclaredAction`，见 `docs/security/01`）。
- `App.tsx` 的 `runChoice`（卡片内选项 widget、随身物品对话框）直接 `airpGateway.choose`；`runEntityAction`（「the player requests …」动作）直接 `submitWriterText`，唯一守卫是作家正在写。
- 理由：把这些改成草稿会让「回到工作室」也要等作家跑一轮，普通选项要玩家再按一次 Send 才落事件——与 doc-21 §5.5「事件落库不触发一轮」并不冲突，触发与否仍由 `autoWrite` 决定。
- 守卫：`apps/web/test/declared-action-web.test.mjs`「every choice click runs through /api/choice」「App runs widget choices and entity actions directly」。
