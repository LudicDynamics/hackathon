...
1. **环境白噪音（Layer 1：Ambient Soundscape）**：
   - 随场景层级切换自动交叉淡入淡出（Crossfade 1.5s）：
     - `world/大地图`：清冷夜风 + 远方钟楼鸣响；
     - `world/旅馆`：室内暖火噼啪 + 沉闷的低声杂音；
     - `world/旅馆/密室`：潮湿水滴声 + 幽闭空灵气流。
...
- **状态形态轻量化**：底层状态完全保持 `doc-05` 的极简约定——收在 chalk frontmatter 的 `status.data` 中（时间/天数/核心进展），随剧情自然流转；
...
1. **0:00 - 0:30【世界内导言·我是谁】**：
   - 玩家就是 Sherlock Holmes；代表自己的 avatar 已经站在 221B 的工作桌前。窗外雨声、壁炉微光、煤气灯和纸面视差同时建立 Baker Street 的空间感；
   - 玩家看见 Watson 的委托信。Watson 直接称呼“Holmes”，并在信中交代当前事件、私人关系和不行动的代价；
   - 玩家打开并接受信件：纸张展开、拟音落下、Chalk 给出第一句回应，现场地址变成可进入的 Gate。
2. **0:30 - 1:30【玩法高潮·我能影响世界】**：
   - 玩家穿过 Gate 进入案发现场，场景、环境声与可操作证物一起落位；
   - 玩家拿起一件证物，将其拖向现场中的机关或关键痕迹，触发调查检定；
   - 全屏压暗，大号象牙骰子翻滚。结果不只显示成败：Chalk 当场改写案发现场，一条新线索和下一处去向被物化到画布上。
3. **1:30 - 2:15【角色记忆·这里的人认识我】**：
   - 玩家点开 Watson，进入遮罩式特写；Watson 把玩家当作 Holmes，不重复任务说明，而是直接回应他刚才拿起的证物、掷骰结果和推断方向；
   - 玩家作出一个承诺或追问，Watson 把它写入两人的持久关系，并把新地点的风险与 Holmes 的共同历史联系起来。
4. **2:15 - 3:00【上帝之手·把可玩性设计进世界】**：
   - 切换上帝模式，展示刚才的“信件 → Gate → 证物 → 检定 → 角色回应”关卡链如何被编排；
   - 在链条末端为世界添加一条受控分支：设定新地点的入场原因、关键危险和必须继承的玩家事实；
   - 回到游玩模式，新地点按该世界的生成语法展开。评委看到的不是“AI 生了一张卡”，而是“设计者创造了一段仍然会生长的关卡”。

这条动线必须在同一个世界、同一条因果上完成；不得为了展示功能在 Baker Street、酒馆和无关角色之间跳切。
