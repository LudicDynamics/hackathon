...
> 日期：2026-09-13。状态：**实现已落地**（2026-09-14 回写）；帧形状、常量与 surface 语义仍按本文冻结，代码落点见 §3 与 §11。
...
- **现状（2026-09-14）**：旧 `writerToolsInFlight` 忙计数已删除（全仓零命中）。`useWorld.ts` 仍保留 `case 'tool_start'` / `case 'tool_end'`，但只服务角色幻影的失败撤离（`apps/web/src/state/useWorld.ts:551,553-562`），不再承载 writer busy。writer busy、phase、stop 的唯一状态源是 `docs/ux/03-Chrome与公开状态.md` 的 `WriterPublicState`（`apps/web/src/lib/writer-state.ts:208` 的 `getWriterPublicState`），activity rail 不得读写该状态。
- 重连路径：writer 状态由 UX03 canonical projection 处理，胶囊状态由本篇的 activity store 处理（`useWorld.ts` 断线时 `agentActivityStore.clearAll()`），二者不得互相代替。
- 事件投递口径：`useWorld.ts` 只认 `case 'x'` 与 `X.type === 'x'` 字面量（`tools/check-ws-contract.mjs` 的 `consumedFrom`），消费文件白名单在 `tools/check-ws-contract.mjs` 的 `CONSUMERS`。
- 帧清单唯一真相源：`docs/tools/12-工具注册与路由统一.md §6.2`（`agent_activity` **已登记**，`docs/tools/12-工具注册与路由统一.md:596`）。
...
- 舞台指示既有文案：`CharacterModal.tsx:74-77` 的 `SILENCE_LINE`（en/ja 各一条，含括号；契约 §5.3 已裁定清洗后为空 → 接受静音，不豁免）。

### 2.5 TTS 清洗的唯一来源（本篇只消费）

`prefetchVoice` 现状（`apps/web/src/components/overlay/CharacterModal.tsx:266-296`）：先 `const text = sanitiseTtsText(page.text)`（`:268`，结果写入 `page.voiceText`），清洗后为空则 `voiceState='failed'` 并直接 `onVoiceResolved` 不发请求（`:269-274`）；服务端在 `apps/server/src/routes/tts.ts:293-299` 幂等重清洗，为空则 400 `invalid_argument`。唯一实现 `packages/shared/src/rules/tts-text.ts:30` 导出 `sanitiseTtsText(raw: string): string`（由 `packages/shared/src/index.ts:29` 转出），前端与服务端 `import` **同一个**，**禁止**前端另存一份括号正则。
...
| `Could not remember` | error | `memory` |
...
| # | 文件 | 符号 / 行 | 改动 |
|---:|---|---|---|
| 1 | `apps/web/src/lib/agent-activity.ts` | 已落地 | 四常量（`:17,19,21,23`）+ 类型 + 纯函数（`normalizeAgentActivityFrame :148`、`upsertActivity :204`、`promoteActivities :235`、`pruneActivities :261`、`visibleActivities :300`、`activityLabel :465`、`activityAriaText :486`、`summariseActivities :506`） |
| 2 | `apps/web/src/lib/agent-activity-store.ts` | 已落地 | `createAgentActivityStore` / 单例 `agentActivityStore` / `resetAgentActivityForTest` |
| 3 | `apps/web/src/state/useAgentActivity.ts` | 已落地 | `useAgentActivity(surface, agentId?)`（§3.4） |
| 4 | `apps/web/src/components/chrome/ActivityRail.tsx` | 已落地 | `ActivityRail`（§3.5） |
| 5 | `apps/web/src/state/useWorld.ts` | `case 'agent_activity'`（`:546-548`） | `agentActivityStore.ingest(msg)`；断线路径加 `clearAll()` |
| 6 | `apps/web/src/App.tsx` | `loadWorld` | 加 `clearAll()` |
| 7 | `apps/web/src/App.tsx` | `closeCharacter`（`:834`） | 加 `clearSurface('character-modal')` |
| 8 | `apps/web/src/App.tsx` | `:1007` | 挂 `<ActivityRail surface="rail" className="prototype-chrome" />`，**任何 activeCharacter/nookChar 状态均保持挂载**；角色活动另挂在 CharacterModal |
| 9 | `apps/web/src/components/overlay/CharacterModal.tsx` | `:838` | `<ActivityRail surface="character-modal" agentId={\`character:${characterId}\`} />` |
| 10 | `apps/web/src/scene-shell.css` | rail 样式区 | §6.1 的全局 rail 样式 |
| 11 | `apps/web/src/index.css` | 角色 rail 样式区 | §6.2 的角色 rail 样式 |
| 12 | `apps/web/src/index.css` | 工具类区 | `.sr-only` 工具类（§3.5） |
| 13 | `apps/web/src/lib/messages.json` | `:160-168,277,282` 等 | §8.2 全部 key × `zh-CN`/`ja` |
| 14 | `apps/web/test/agent-activity.test.mjs` | 已落地（LC1–LC10 全绿） | 断言集（§10）；命名与落点已与 04 对齐 |

### 9.2 与 TTS 清洗的接缝（只消费，不实现）

`CharacterModal.tsx:266-296` 的 `prefetchVoice` **已按此接线**（下方代码为现状，非提案）：
...
> **一致性自查（2026-09-14 复核）**：本篇的代码锚点已按当前 `main` 工作树回写 —— `useWorld.ts:546-548`（`case 'agent_activity'`）、`:551,553-562`（`tool_start`/`tool_end`）；`App.tsx:834`（`clearSurface`）、`:1007`（全局 rail）；`CharacterModal.tsx:74-77`（`SILENCE_LINE`）、`:266-296`（`prefetchVoice`）、`:838`（角色 rail）；`dialogue-pages.ts:121,136`（turn buffer）。行号漂移时以符号名为准。
...
> 2026-09-14 回写：本节「本批后」一列**已全部成立**（`apps/web/test/agent-activity.test.mjs` 的 LC1–LC10 全绿）；旧现状列保留为对照。

| # | 迁移前现状 | 本批落地（已实现） | 依据 |
|---:|---|---|---|
| 1 | 无任何 activity rail；工具动作只体现在已删除的 `writerToolsInFlight` 忙计数（不可见） | 两条 rail 显示归一化胶囊（`ActivityRail.tsx`，两处挂载 `App.tsx:1007`、`CharacterModal.tsx:838`）；writer busy 仍由 UX03 `WriterPublicState` 提供 | 契约 §2.2 / UX03 |
| 2 | `useWorld` 无 `agent_activity` case | `useWorld.ts:546-548` 的 case + `agentActivityStore.ingest(msg)` | 契约 §3 冻结要求 |
| 3 | 关闭角色只清 `activeCharacter` | 追加 `clearSurface('character-modal')`（`App.tsx:834`） | 契约 §7.2 |
| 4 | 切世界只清本地 UI 状态 | 追加 `clearAll()` | 契约 §7.2 反例（下一角色不继承） |
| 5 | 断线只重置 footprint 计数与 writer 状态 | 断线路径追加 `clearAll()` | 契约 §7.2 断线等价形 |
| 6 | writer 进度只有 "作家正在续写…" + 秒数 | 生命周期提示**保留**，另加动作胶囊（并存） | 契约 §6、§7.3 |
| 7 | `prefetchVoice` 用 `page.text` 直接发请求 | 先 `sanitiseTtsText`，空则跳过、走无语音 stinger（`CharacterModal.tsx:268-274`） | 契约 §5；**由 `01` 定义，本篇只消费** |
| 8 | `index.css` 无 `.sr-only` 工具类 | 已新增（rail 的 sr-only 摘要行使用） | §3.5 |
| 9 | `messages.json` 无 activity 文案 | 已新增 §8.2 三张表 × 3 locale（`messages.json:160-168,277,282` 等），由 LC10 直接断言 | §8.2 |

---

## 12. 必须回写的文档与门禁

| # | 文档 / 门禁 | 回写内容 | 状态（2026-09-14 复核） |
|---:|---|---|---|
| W1 | `docs/tools/12-工具注册与路由统一.md §6.2` | 增加 `agent_activity` 行：载荷（共同契约 §3 逐字）、发射方、前端消费方；注明它**不替代** `tool_start`/`tool_end`、不落事件 | **已完成**（`docs/tools/12-工具注册与路由统一.md:596`） |
| W2 | `tools/check-ws-contract.mjs` | §6.2 表即真相源；加行后 `emitted`（`event-bridge.ts` 广播）与 `consumed`（`useWorld.ts` 的 case）三侧对齐 | **已完成**：`agent_activity` **未**进 `INTENTIONALLY_UNCONSUMED`（`tools/check-ws-contract.mjs:64-71`）与 `KNOWN_DARK`（`:75-88`）；`node tools/check-ws-contract.mjs --json` 现为 `findings: []`，`contract: 27 / emitted: 27 / consumed: 24` |
| W3 | `docs/wiring/前端接线体检.md` | 登记 `agent_activity` 的消费面 | **未完成**（该文件当前完全未提 activity） |
| W4 | `AGENTS.md §2/§6.3` | 目录树补 `docs/agent-awareness/`、`apps/web/src/lib/agent-activity*.ts`、`apps/web/src/components/chrome/ActivityRail.tsx`、`apps/web/src/state/useAgentActivity.ts`、`packages/shared/src/rules/tts-text.ts` | **部分完成**：`AGENTS.md:79` 已有「Agent activity、角色 turn、TTS 动作过滤 → `docs/agent-awareness/00`」导航行；§2 目录树与 §6.3 同步表的逐路径登记**仍未补** |
| W5 | `docs/doc-06-演出与交互设计.md` | 底部 rail 的视觉规格（两级材质、不遮挡清单） | **未完成** |
| W6 | `docs/perform/00-共同上下文.md` | writer/functional activity rail 与既有演出帧可并行展示 | **未完成**（`docs/perform/00` 的逐帧发布点 `:53` 与现有 case 枚举 `:58` 仍未纳入 `agent_activity`） |
| W7 | `docs/tts/03-遮罩分页与演出.md` §3.1/§5.1 | `prefetchVoice` 的 body 改为清洗后文本、空则跳过 | **未完成**（本篇为消费方确认，`01` 主责） |
 
**i18n 门禁**：`pnpm check:i18n` 会因 §8.2 的新 key 是**动态调用**而**扫不到** → 不产生红/绿信号；因此 `apps/web/test/agent-activity.test.mjs` 的 LC10 是这些 key 的**唯一** mechanical 证据（§8.5），且**已落地**（该文件的 `LC10 every label key exists in messages.json with non-empty zh-CN and ja`）。

**WS 门禁现状**：`node tools/check-ws-contract.mjs --json` → `findings: []`（`contract: 27 / emitted: 27 / consumed: 24`），本帧三侧已对齐，归零达成。
