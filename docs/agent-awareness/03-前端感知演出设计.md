...
- **唯一 WS 消费点**：`apps/web/src/state/useWorld.ts:350` 的 `onMessage`；`:356` 把**每个原始帧**原样派发到 `window` 的 `CustomEvent('airp:agent-frame')`；`:408` `case 'tool_start'` / `:411` `case 'tool_end'` 目前**只**服务 writer footprint 忙计数（`writerToolsInFlight`）；`:423` 把 `character_delta|character_message|character_idle` 转给 `airp:character-frame`。
- 重连路径：`useWorld.ts:552-562` 的 `connect` / `ws.onopen`（重置 `writerToolsInFlight`、`resetWriter()`、重取层）/ `ws.onclose`（1.2s 后重连）。
...
- **漏了会怎样**：断线时正在跑的 `running` 胶囊永远等不到 terminal，要 90s 才转超时失败并再停 4.5s；期间玩家看到一条早已无效的"正在写入…"。与既有 `writerToolsInFlight.current = 0` / `resetWriter()` 的处理理由逐字相同（`useWorld.ts:554-558` 注释）。
- 断线**硬清**（无残留、无淡出尾巴）；关闭角色**收束后淡出**（4.5s 尾巴，但组件已卸载故不可见）。
...
| # | 现状 | 本批后 | 依据 |
|---:|---|---|---|
| 1 | 无任何 activity rail；工具动作只体现在 `writerToolsInFlight` 忙计数（不可见） | 两条 rail 显示归一化胶囊 | 契约 §2.2 |
