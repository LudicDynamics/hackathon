...
5. `docs/ui/前端改造计划.md`（施工单，**其中 T2.1 的"作家 mood 标签"一条已被本文 §12 推翻**）。
...
2026-09-13 用户确认：仅增加独立音量控制。`getChannelVolume / setChannelVolume` 控制 music（层 BGM、主题曲及 BGM 合成兜底）与 voice（TTS）两个增益总线，均在 master 之前；环境音和拟音不受这两个滑杆影响。音量 0–1，本浏览器持久化，默认 1 保留已有混音比例；调节不重启音轨、不改变 crossfade 或语音中断逻辑，总静音仍关闭全部声音。入口为 Settings → 音量。
