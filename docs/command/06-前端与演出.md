...
**实测机制**：作家注入的事件查询**必然排除 actor == 自己**的事件——`packages/shared/src/inject/collect.ts:250-252` 传 `excludeActor: actor`，`local-store.ts:837-840`（旧标 `:745-748`，**快照已漂移**） 逐字 `NOT (actor_type = ? AND COALESCE(actor_id, '') = ?)`。而契约 §5.1 冻结"命令事件的 actor = 触发者"。两者相乘得出：
...
| 新卡以 path 为 key（稳定挂载） | `apps/web/src/components/canvas/Canvas.tsx:645`（旧标 `:593`；更早 `:544-547`，**已漂移**） |
