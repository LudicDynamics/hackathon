...
> 不拥有：幂等机制（→ `05`）、效果执行（→ `04`）、命令文件 schema（→ `01`）、触发点与绑定（→ `02`）。
...
export interface WorldCommandStepOutcome {
  ...
  ok: boolean;
  /** 每个目标的动作事件 id，**与 targets 同序**。`link` 不落事件（`canvas.ts:204-250`）→ 该步此字段缺省，**不是 bug，UI MUST NOT 因此判失败**。 */
  eventIds?: string[];
  /** 失败的说明；给出**缺什么**，不是"失败了"。 */
...
/**
 * 一次触发里全部命令的汇总。挂在 `ActionResult.details.commands`。
 * 注意：**没有 `commands` 键与 `commands: []` 是两件事**——
 * 前者 = 这个实体没有声明 `on.<hook>`；后者 = 声明了但没命中。
 * UI 只在前者时保持沉默（§7.0 的"没有承诺就不欠账"）。
 */
export type WorldCommandReceipts = WorldCommandReceipt[];
```
...
**为什么顶层摊平而不是嵌套**：`reply` 已经做 `res.json({ ok: true, ...details })`（`apps/server/src/routes/world.ts:74-80`），摊平即零路由改动；而 `parseRollDiceDetails` 对 details 是白名单重建（§8.2 红线 2），摊平让"多出来的键"一眼可见。

### 2.2 持久回执：实体 `.md` frontmatter 的 `command_log` / `command_error`

契约 §3.3 冻结：这两个 key 写在**被触发的实体**上，且它们**不随命令文件形态变更**（命令文件已改为 `command/<id>.yaml`，实体仍是 `.md` + frontmatter）。它们是文件的真相，也是玩家复访时唯一还看得见的东西——toast 3 秒后消失（`apps/web/src/App.tsx:219-226`）。

**本节只描述前端要读什么。形状的权威定义在 `05` §2.2**；下面是 `06` 渲染所依赖的那个子集，字段名与 `05` 对齐：

```yaml
# world/london-map/04-investigation-dice.md 的 frontmatter（示意；权威形状见 docs/command/05 §2.2）
command_log:
  - commandKey: 9f2c1a7b4e0d3c58        # 幂等真相源（05 §3.1 的 sha256 前 16 位）
    settle: ran                          # 05 的四 verb：ran|reused|resumed|failed
    plan: 4b81f0a2                       # 命令 do 的规范化摘要，8 hex，**仅溯源**
    at: "2026-09-14T09:12:03.441Z"       # 服务端 now()
    steps:                               # `step` = 扁平序（续跑游标，MUST 从 0 连续）
      # list-args：do[0] 的 give 拿整个数组 → 每项各占一个 step（05 §5.7）
      - { step: 0, decl: "do[0]#1", action: give, ok: true,
          targets:  [world/london-map/investigation-great-success.md], eventIds: [evt-1043] }
      - { step: 1, decl: "do[0]#2", action: give, ok: true,
          targets:  [world/london-map/investigation-notes.md],        eventIds: [evt-1044] }
      # fold-args：do[1] 的 edit 恒只占一个 step，但一次写多个目标
      - { step: 2, decl: "do[1]#1", action: edit, ok: true,
          targets:  [world/london-map/04-investigation-dice.md],      eventIds: [evt-1045] }
command_error:
  - commandKey: 3d7e0b91c4a25f60
    at: "2026-09-14T09:12:03.441Z"
    step: 1                              # 扁平序游标，不是 do[] 下标
    action: move
    code: command_target_missing
    target: player/lamp-oil.md           # 06 请求增补的字段（§11.7）
    pending: true                        # 这一步没做成 → 世界是不完整的
    message: "move: \"player/lamp-oil.md\" does not exist."   # ActionError.message 原文，上限 1000 字符截断
```
...
| `command_log[].steps[].targets` | 数组 | 这一步改写了哪些路径（`fold-args` 时 >1） | 非数组 → 该行不列路径，只显示 `action` |
| `command_log[].steps[].decl` | 字符串 | `do[0]#2` 形态的声明位置 | **不显示**（是溯源信息）；要显示时只在 hover title 里 |
| `command_log[].steps[].ok` | 布尔 | 这一步是否全部落地 | 非布尔 → 按 `true` 处理（保守：不虚报失败） |
...
| `command_error[].code` | 字符串 | 选玩家文案（§7.1 的查表键） | 未登记的 code → 用兜底文案（"没能执行"），**MUST NOT** 把 `message` 当兜底文案 |
| `command_error[].message` | 字符串（≤1000） | **只进模型/调试**，不进玩家 UI | 见下方红线 |
| `command_error[].pending` | 布尔 | `true` → 世界处于不完整状态，文案用"只做到一部分" | 非布尔 → 按 `true` 处理（保守：宁可说得更谨慎） |

**一条红线**（与 §7.1 纪律 1 同源，这里是它在渲染层的落地）：`command_error[].message` 是 `ActionError.message` 原文，可能含路径、行号、内部词汇。**玩家 UI MUST NOT 渲染它。** 玩家的文案由 `code` 查表得到（§7.1 的右列），模型/调试侧才读 `message`。

> 为什么这条要单独写：`message` 是**唯一一个"看起来就该直接显示"的字段**——它已经是一句人话了，就摆在 `command_error[]` 里，任何实现者顺手就会用它。但它正是那条把 `line 7, column 3: unknown top-level key "rewardz"` 泄露给玩家的路径。
...
**长度上限是 UI 约束**：卡片上的折叠区放不下更多，且只有"最新的失败"需要被看见。上限的**具体数字由 `05` 定**，本文只要求"有上界且超出行为是丢最旧、不是无限增长"，以及**前端在数组超长时不报错、只渲染最新的 N 条**。
...
export function parseCommandReceipts(raw: unknown): WorldCommandReceipts;
/**
 * 把回执压成玩家要的那一句话。空数组 → null（UI 保持沉默）。
 * `t` 由调用方注入（i18n.ts:30 的 useLocale 形态），本模块不碰 React。
 */
export function receiptLine(
  receipts: WorldCommandReceipts,
  t: (key: string, values?: Record<string, string | number>) => string,
): { tone: 'ok' | 'notice' | 'warning'; text: string; detail?: string } | null;
/** 载荷指纹，供与 05 的幂等对齐；status 参与，因为它决定玩家看到什么。 */
export function receiptsFingerprint(r: WorldCommandReceipts): string;
```

`parseCommandReceipts` 是**宽松**的（畸形条目被丢弃、返回可能为空的数组），这与 `parseRollDiceDetails` 的**严格**相反（`apps/web/src/lib/dice-ceremony.ts:65-90`）——理由见 §7.3。
...
| # | 步骤 | 落点 | 漏了会怎样 |
|---|---|---|---|
| 1 | 点击即锁按钮 | `DiceRoller.tsx:44-45,116-118` 的 `busy` 守卫 | 双击产生两个 `req:<uuid>` turn 与两条 `roll_resolved`（第二条被 `dice_already_rolled` 拒），玩家看到"已经掷过"的错误，而他认为自己只点了一下 |
| 2 | 请求发出 | `world.ts:1102-1119` | — |
| 3 | 命令在**同一个请求内**跑完 | 触发点（`02`）；契约 §4.3 | 若改成"响应先回、命令异步跑"，玩家看到骰子结果却看不到后果，且没有任何一次返回能证明后果发生过（违反硬门 1 的可复访要求） |
| 4 | 响应/工具返回值带回 `details.commands` | §2.1；`reply` 摊平（`world.ts:79`） | 回执只能靠 `world_event` 补 —— 落到 §3.3 的坑里：慢，而且剧透 |
...
| 7 | **`file_changed` / `world_event` 到达，被 reveal gate 挡住** | NEW，`useWorld.ts:424-425,427-449`（§8.3） | **剧透**：奖励卡在骰子还在转的时候就出现在模糊背景后面（§3.4 算出 150ms 与 10~50ms 两个到达时刻，都远早于 1200ms） |
...
| 11 | `releaseReveal()` → **一次**合并重取 | NEW，§8.3 | 不合并就有 1 + N 个请求排队（`file_changed` 一次 + N 条 `world_event` 各一次）。`reqSeqRef` 只保留最后一个（`useWorld.ts:171-175`），但**每一次都在服务端跑一遍** `seatUnplaced`/`reseatLayer` 两个写事务 |
| 12 | 新奖励卡入场 | 现有 `Canvas.tsx:544-547`（`key={item.path}` 稳定）+ 一条 CSS 入场动画 | 无动画则"啪"地出现；动画若依赖测量（读 `offsetHeight`）则违反 `AGENTS.md:104` |
...
`apps/server/src/routes/world.ts:702-892` 一次调用要（**全部是必做的，不是浪费**）：

| 动作 | 行 |
|---|---|
| 递归列出本层卡片 | `:707` `pageOfLayer` |
| 每张卡 `readFile` + `parseFrontmatter` | `:710-715`（模板 `london-map` 实测 5 张卡；`world/` 下 6 个条目） |
| 每个子目录 README 再读一次 | `:716-729` |
| **`store.getEvents(1000)` 扫最近 1000 条事件** | `:740-756` |
| `store.getLayerCards` | `:738` |
| **`seatUnplaced`（`BEGIN IMMEDIATE` 写事务）** | `:781` |
| **`reseatLayer`（再一个写事务）** | `:787-792` |
| `appearanceContext` | `:794` |
| 本层 README 再读一次（非 map 层还读一次 map README） | `:830-867` |
| 两条 canvas SQL | `:872-890` |
...
t=R+10~50ms  ★ world_event 到达（fs.watch 立刻 kick drain，
              event-bridge.ts:547；尾读 1s timer 只是启动窗口的兜底，
              TAIL_POLL_MS=1000，:16,500-503）[推断]
t=R+150ms    ★ file_changed 到达（WATCH_DEBOUNCE_MS=150，
              event-bridge.ts:18,550-558）
...
`prefers-reduced-motion`（`DiceCeremony.tsx:25,64`）把 `rolling` 压到 180ms，`SETTLE_MS` 不变。这反而让"数据早于演出到达"的结论**更强**：180ms 的翻滚期更盖不住 150ms 的 `file_changed`。
...
- 新奖励卡的位置**由服务端排座给出**（`seatUnplaced`，`world.ts:781`），前端 MUST NOT 为了"让新卡落在好看的格子"而回写坐标。`AGENTS.md:103` 的红线逐字：「`Canvas` 的取景 effect 只能移动相机，不能把首帧排版结果回写卡片坐标；否则首次拖拽可能让整层跳位。」
- 入场动画 MUST NOT 读 `offsetHeight` / `getBoundingClientRect`：`AGENTS.md:104` 禁止在拖拽循环内读取会强制同步布局的属性，而入场动画与拖拽是同一批卡上的并发活动。用纯 CSS keyframes（`transform` + `opacity`），不测任何盒子。
- 失败卡的 `command_error` **默认展开**会让该卡一进来就比同类卡高。这是可接受的：`POST /api/card/footprint`（`useWorld.ts:347-355`）本来就为"把实测高度写回"而存在，而 `reseatLayer` 只为"声明尺寸或实测尺寸**合法漂移**了的卡"重排（`world.ts:785-790`）。但这意味着**失败卡的入场动画更不能用高度驱动**。
...
| actor | = **触发者**（玩家掷骰 → `player`），不是 `engine` | 契约 §5.1；`packages/shared/src/actions/actor.ts:9` |
...
| `roll_resolved.detail` | 恰好 8 个键，**不含** `rolls`/`crit`/`fumble`/`forged` | `roll-dice.ts:225-233`；`events.ts:80-90` |

**关于 turn 的一条修订（契约 §10.7 已证伪原表述）**：`serviceFor` 的 `randomUUID()` 在**函数体内**（`apps/server/src/routes/world.ts:63-65`），所以**每调用一次就铸一个新 turn**。`POST /api/choice` 先 `runDeclaredChoice(serviceFor(...))`（`world.ts:1225`），未命中再 `serviceFor(...).chooseOption(...)`（`:1227`）——**两次** `serviceFor` → **一次 HTTP 请求产生两个 turn**。

**对前端的后果（这条才是 `06` 要管的）**：

- 命令回执与触发它的动作**一定**同 turn（命令在该次调用内同步跑完），这条**不受影响**；
- 但 **MUST NOT 用 turn 做"同一次点击"的前端去重键**。骰子链路今天用的是 `fingerprint`/`requestKey`（`dice-ceremony.ts:204-219`）而不是 turn，**这是对的，保持不变**；
...
**为什么 `detail.by` 被删掉（这是一条"埋着没炸的雷"，必须写下来）**：`detail.by` 已被 `layer_initialized` 占用，且是**闭枚举** `z.enum(['writer','player','engine'])`（`packages/shared/src/schemas/events.ts:99`），而 dev 模式下 `appendEvent` 会对 detail 跑 `safeParse`（`packages/shared/src/store/local-store.ts:668-679`）。给命令事件写 `by: 'command'` 会在 dev 模式直接炸。今天没炸只是因为 `04` 冻结的七个效果里没有 `recordLayerInitialized`——**条件性安全**。
...
**前端可以从事件表里拿什么、不能拿什么**：`world_event` 帧带完整 `WorldEvent`，前端**只用来决定"重取"**（`useWorld.ts:427-449`）。`roll_resolved` 不进演出入口（`useWorld.ts:654-656` 明确忽略历史事件）——**这条必须保持**，否则复访一个世界会把历史骰子重新演一遍。
...
| 帧 | 改动 | 理由 |
|---|---|---|
| `dice_result` | **加一个可选顶层字段 `commands`** | `event-bridge.ts:284-298` 从 `result.details` 逐个搬运，容器已经在手上 |
...
**实测机制**：作家注入的事件查询**必然排除 actor == 自己**的事件——`packages/shared/src/inject/collect.ts:250-252` 传 `excludeActor: actor`，`local-store.ts:745-748` 逐字 `NOT (actor_type = ? AND COALESCE(actor_id, '') = ?)`。而契约 §5.1 冻结"命令事件的 actor = 触发者"。两者相乘得出：
...
玩家 HTTP 路径 → 回执在响应里。作家工具路径 → 只有 `roll_dice` 有 `dice_result` 帧（`event-bridge.ts:279-298`）；`choose` / `use` 工具**没有对应的演出帧**。
...
**但立一条硬约束**：`useWorld` 的 `case 'file_changed'` 与 `case 'world_event'`（`useWorld.ts:424-425,427-449`）触发重取时 **MUST NOT** 附加任何"新卡提示音 / 闪烁"。否则作家路径会突然冒出玩家从未发起过的演出反馈。

### 6.3 前端消费点全表（改动后）

| 入口 | 位置 | 改动 |
|---|---|---|
| 玩家掷骰 HTTP 响应 | `DiceRoller.tsx:53-74` | 多解析一个可选 `commands` 字段 |
| `dice_result` 帧 | `useWorld.ts:584-587` → `App.tsx:300-326` | 帧里多一个可选 `commands`；`markPlayed`/去重逻辑**不变** |
| `world_event` | `useWorld.ts:427-449` | fetchLayer 改走 reveal gate（§8.3） |
| `file_changed` | `useWorld.ts:424-425` | 同上（**这条最容易漏，它比 `world_event` 先到**） |
| 卡片上的持久回执 | `fm.tsx:26-177` 的 widget 序列 | 新增 `command_error` / `command_log` 渲染（§7.5） |
...
| **③ 命令写错**（YAML 非法 / 文件不存在） | `skipped` | `conflict` | 同上，但文案是"世界里的自动后果没能执行"，**绝不显示解析器原文** | 未改；骰子照掷、`roll_resolved` 照落 |
...
`code` 的最终命名归 `05`（它拥有失败口径）；下表是 `06` 对**可见性**的要求。**本表同时是 `action-feedback.ts` 的改动输入**：`CONFLICT_CODES`（`apps/web/src/lib/action-feedback.ts:36-40`）与 `REJECTED_CODES`（`:41-44`）今夜都不含这些码，落不进去就会一律兜底成 `failed`（`:108`），玩家看到红色"执行失败"而实际只是"世界拒绝"。
...
| `command_disabled` | `skipped` | **CONFLICT** | `World command "investigate-clue" is disabled in this world. The roll is unaffected.` | `The automatic outcome for this check is switched off in this world. Your check still counts — the story goes on.` |
...
| `command_missing` | `skipped` | **CONFLICT** | `Entity declares on.roll → "investigate-clue" but command/investigate-clue.yaml does not exist. Create it with parseWorldCommand-valid YAML.` | `The world is missing a rule this check refers to. Your check still counts — the story goes on.` |
| `command_invalid` | `skipped` | **CONFLICT** | `command/investigate-clue.yaml line 7, column 3: unknown top-level key "rewardz". Allowed: name, desc, params, do.`（行列位置归 `01`） | `The world's rule for this check is written wrong, so the automatic outcome did not run. Your check still counts.` |
...
`apps/web/src/lib/action-feedback.ts:3` 逐字 `['accepted', 'conflict', 'rejected', 'failed']`。命令的全部结果映射到这四态：
...
`parseRollDiceDetails` 是**严格**的（`dice-ceremony.ts:65-90`）：12 个键必须齐、类型必须对，否则返回 `null`，玩家看到 `DiceRoller.tsx:68` 的 `missing authoritative roll details`。这个严格是对的——**骰子细节错了，整个仪式没有意义**（没有 `rolls` 就没有骰面可演）。
...
- **插入点**：`DiceCeremony.tsx:140-160` 的 `phase === 'settled'` 分组，夹在 `badge(input.passed)`（`:153`）与 `DONE` 按钮（`:154-159`）之间。
...
- **一次 entry 命中多条命令**（`on.roll` 里区间相交，`05` 的 `overlapping_command_when` 场景）时，汇总行只说总数；**MUST NOT** 因为其中一条 `reused` 就把整体说成"没给"。聚合规则按 `status` 定（**不按 `settle`**——verb 是结算器视角，`ran` 既可能是 `applied` 也可能是 `failed`）：有任意一条 `applied`/`resumed` → `ok` 语气；否则全部 `reused` → `conflict` 语气；否则按 `failed` > `blocked` > `skipped` 取最严重的一条定 tone。

### 7.5 卡片上的持久回执（失败必须撞到眼里）

渲染点：`apps/web/src/lib/fm.tsx:26-177` 的 widget 序列。放在 `dice` 块（`:128-138`）之后、`choices` 块（`:140-170`）之前——**骰子、后果、下一步按钮**，与玩家的阅读顺序一致。
...
| 复用 | 位置 | 怎么用 |
|---|---|---|
| `dice-outcome-letter` 的折叠信封样式 | `CardRenderer.tsx:118-121`；`declared-actions.css:180-213` | `command_error` / `command_log` 共用它的排版骨架（padding / border / `max-height` / `<summary>` 折叠），只加一个 tone 变体类（`.dice-outcome-letter--error`）改边框与标题色。**不新建一整套样式表** |
| `fm.tsx` 的 widget 序列 | `fm.tsx:99-126`（`status` 那块的折叠按钮就是同一族写法） | 新 widget 沿用同一交互语（`<details>` / `Chevron`） |
| `useLocale().t` | `i18n.ts:10-31`（key = en 文案） | 所有新增 UI 文案走它 |

**奖励信默认折叠、失败默认展开的刻意不对称**：

- `dice_reward` 是**奖励**，折叠着等玩家点（`CardRenderer.tsx:118` 的 `<details>` 无 `open`）。
- 失败必须主动撞到玩家眼里，否则等于静默。**默认展开是硬门 4 在这个组件里的落地形态。**

**为什么必须是卡片而不是 toast**（现状 `App.tsx:219-226`：`setToast` + 3 秒后清空）：

1. **3 秒后消失**。而仪式是模态遮罩（`DiceCeremony.tsx:102-107` 的 `fixed inset-0 z-50`），失败回执在仪式里出现的同一时刻，toast 在其下方且在同一秒开始倒计时。
...
5. 源卡上 `command_error` **默认展开**，常驻。刷新后仍在（它是 frontmatter）。作家下一轮读到它（归 `10`）。
...
```text
packages/shared/src/commands/receipt.ts                      NEW（§2.1 的类型）
...
apps/server/src/engine/event-bridge.ts:284-298               dice_result 帧加 commands 字段
```

`dice_result` 帧的改动（在既有 push 对象里加一段）：

```ts
// apps/server/src/engine/event-bridge.ts:284-298 —— 只加字段，不新增帧
push({
  type: 'dice_result',
  source,
  path: details.path,
...
**路由零改动**：`reply` 把 `details` 摊平（`world.ts:79`），`commands` 自动进响应体。

### 8.2 前端改动清单

```text
apps/web/src/lib/world-command.ts                            NEW（§2.3 的守卫 / 文案 / 指纹）
...
apps/web/src/state/useWorld.ts:424-425                       case 'file_changed' 的 fetchLayer 走 gate
apps/web/src/state/useWorld.ts:448                           case 'world_event' 的 fetchLayer 走 gate
...
apps/web/src/lib/action-feedback.ts:36-44                    CONFLICT_CODES / REJECTED_CODES 加 §7.1 的码
apps/web/src/lib/fm.tsx:26-177                               command_error / command_log widget
apps/web/src/components/narrative/declared-actions.css:180   .dice-outcome-letter 加 tone 变体类
apps/web/src/lib/messages.json                               新增文案的 zh-CN / ja（key = en，i18n.ts:10-13）
apps/web/src/lib/d10-display.ts / dice-ceremony.ts:277-280   §11.3 / §11.4 的对策
```

**三条红线（每条都能独立造成玩家可见的 bug）**：

1. **`commands` MUST NOT 加进 `dice-ceremony.ts:54` 的 `detailsKeys`。** 那个数组是**必填键**列表（`:67` 对每个 key 做 `hasOwn`）；加进去以后"没有命令的普通掷骰"会让整个守卫返回 `null`，玩家看到 `DiceRoller.tsx:68` 的 `missing authoritative roll details`，**骰子仪式彻底不演**。这是本次改动最容易犯、后果最重的一个错。
...
3. **`stageCeremony` 的 legacy 重建（`:281-300`）也要带 `commands`。** 它从 `input` 重建一个 `DiceFrameVerdict`（`:283-296` 逐字段列举）；漏了这一步，`commands` 在 legacy 别名路径上静默消失，而 `App.tsx:950` 的 `verdict={ceremony.verdict}` 正是走那条路。
...
> **为什么"两处"不是保险起见，而是必需——`link` 效果给出了实证。** `04` 冻结的七个效果里，`link`（→ `linkCards`）**不落事件**：`packages/shared/src/actions/canvas.ts:204-250` 只写 canvas link 并 return，全程没有 `appendEvent`（这也正是契约 §10.14(b) 把 `unlink` 移出集合的同一条理由）。所以一条**只含 `link` 的命令**：
>
> - **不会**产生 `world_event`（没有事件可尾读）；
> - **会**产生 `file_changed`——它写的 `.airpworld/canvas.db`（`packages/shared/src/store/local-store.ts:122`）不在 watcher 的忽略名单里（只忽略 `.airpworld/assets/` 与 `.airpworld/sessions/`，`event-bridge.ts:536-540`）。
>
> 结论：**`link` 的可见性完全依赖 `file_changed` 这一条**。只 gate `world_event` 会让它在仪式播放期间就把新连线画到模糊遮罩后面；只 gate `file_changed` 又漏掉其余六个效果的事件路。两条都盖住才是完整的闸。

```ts
// apps/web/src/state/useWorld.ts:424-425 —— file_changed（先到，最容易漏）
case 'file_changed':
  // 命令/骰子落盘会在 ~150ms 触发这条；仪式期间压住，收场后重取一次。
  deferReveal(() => { void fetchLayer(layerRef.current); });
  break;

// apps/web/src/state/useWorld.ts:448 —— world_event
// （上一行 :435 的 forwardWorldEvent 与 :443-447 的 layer-init 分支都不动）
deferReveal(() => { void fetchLayer(layerRef.current); });
break;
```

```tsx
// apps/web/src/components/performance/DiceCeremony.tsx:71-87 的 settled effect
...
**gate 的代价（如实登记）**：仪式播放期间世界视图冻结最多 2.5 秒。这**不是新增的损失**——全屏模态遮罩（`DiceCeremony.tsx:102-107`）本来就把画布挡得只剩一坨模糊背景，此时更新与否玩家都看不清。切层会立即收场（`App.tsx:323-325` 的 `clearCeremony`），gate 随 `releaseReveal` 放行，不会有跨层残留。
...
现状已有一套成熟的"预置卡"机制：`lib/phantom.ts` + `PhantomLayer.tsx` + `phantomSeatFor`（`lib/phantom-seat.ts`），重取后用 `reconcileLanded` 把已到位的幻影撤掉（`useWorld.ts:190`）。看起来该复用，实测结论是**不该**，三条理由：
...
| # | 现状 | 位置 | 本批目标 | 不改的代价 |
|---|---|---|---|---|
| 1 | 玩家掷骰的可见后果**只能**来自整层重取 | `useWorld.ts:424-425,448` | 回执搭同步响应；重取被 gate 压后到仪式收场 | 契约 §1.1 的"毫秒级"在玩家侧不成立（§3.3） |
| 2 | `details` 守卫是**白名单重建**，额外字段静默丢弃 | `dice-ceremony.ts:54,65-90` | 显式吸收可选 `commands`；`commands` 不进必填键列表 | 服务端加了字段也到不了 UI，且**没有任何报错** |
| 3 | `dice_result` 帧搬 11 个字段，不搬 `forged`（有意） | `event-bridge.ts:284-298` | 加搬一个 `commands`（可选、有意，写进文档） | 作家/角色触发的命令对玩家侧无回执 |
| 4 | 失败只能靠 toast（3 秒消失） | `App.tsx:219-226` | 卡片常驻 `command_error`，默认展开 | 硬门 4 不成立（§7.5） |
| 5 | 前端只有四态，码集不含命令码 | `action-feedback.ts:36-44` | 命令码进 CONFLICT/REJECTED 集，**不新增第五态** | 一律兜底 `failed`，玩家看到误导性的红色"执行失败" |
| 6 | 工具路径的后果**看不见**（`excludeActor` 把自己过滤掉） | `inject/collect.ts:250-252`；`local-store.ts:745-748` | 后果走 `details` → tool_result（§6.2a） | 作家重复发奖 |
| 7 | `diceStageDisplay` 只支持 4 种骰型，其余落 tiles | `d10-display.ts:19-50` | 不限制作者；修 fallback 的诚实性（§11.3） | 命令推荐任意 `NdM` 后翻滚期显示假骰面（§10.6） |
| 8 | `rollingFace` 的滚动上限对 `d20` 等是错的 | `dice-ceremony.ts:277-280` | 上限取自 `parseDiceType().faces` | 同上 |
| 9 | `1d100+10` 的 D10Stage reading 显示 `40`、大数字显示 `50` | `d10-display.ts:64-76` vs `DiceCeremony.tsx:151` | 见 §11.4 | 玩家盯着骰面读档位，与发奖依据不一致 |
...
**前置**：起服务 + 前端（`pnpm dev:server` + `pnpm dev:web`）；`templates/whitechapel-jp`，`world/london-map/04-investigation-dice.md` 已声明 `roll_dice` 与 `on.roll`，档位 `13..60` 指向一条 `give`。DevTools 开 Network 与 `Elements`。
...
assert.equal(classifyActionResult('dice', { ok: false, code: 'command_disabled' }), 'conflict');
...
**修复前**：前四条红——命令码不在 `CONFLICT_CODES`/`REJECTED_CODES`（`action-feedback.ts:36-44`），走 `:108` 兜底 → `failed` → 玩家看到红色"执行失败"（`EntityInteractions.tsx:121-124` 的 `setError` 路径），而事实是"已经结算过了，没重复发奖"。

### 10.4 复访不重演（浏览器 + 文件）

1. 掷一次，看到回执与奖励卡；刷新页面。
2. 断言：**没有**骰子仪式（`roll_resolved` 不进演出入口，`useWorld.ts:654-656`）；`command_log` 仍在卡片上；奖励卡仍在。
3. 再点一次掷骰：按钮仍被 `rolled` 挡住（`DiceRoller.tsx:45`），且 `history.db` 的 `max(seq)` **前后一致**（没有任何新的世界写入）。
...
把世界命令关掉（`[C-4]` 未拍板；用 `09` 的开关，或临时删掉 `command/` 目录）。

| 断言 | 值 |
|---|---|
| `POST /api/dice` | `200`，`ok: true` |
| 响应里 `commands[0].status` | `'skipped'` |
| 响应里 `commands[0].code` | `'command_disabled'`（或 `'command_missing'`） |
| 实体文件 | `roll_dice.result` **已写入**、`passed` 已写入 |
| 事件表 | 新增恰好 1 条 `roll_resolved`，`actor.type === 'player'` |
| 仪式 | 完整播放：`rolling` → `settled` → `DONE` |
| 卡片 | `command_error` 存在且**默认展开**，文案是"被关掉了"不是"出错了" |
| 奖励卡 | **不存在**（不伪造） |
| 作家 | 仍能叙述（下一轮注入正常） |

**修复前**：`commands` 字段整个不存在，前端无从得知"本该有后果但没跑"——玩家看到一次干净的骰子结果，不知道自己错过了什么。**这就是静默**，也正是硬门 7 与硬门 4 的交点。
...
- `commands[0].status === 'skipped'`、`code === 'command_invalid'`；
...
**修复前**：`rollingFace`（`dice-ceremony.ts:277-280`）的 `ceiling = value > 6 ? 100 : 6`——`1d20` 会滚出 `53` 这种**物理上不可能**的值，而落定显示 `12`。玩家在翻滚期看到的是假骰面。
...
本节共 7 条。**§11.2 与 §11.3 是契约直接指派给 `06` 的**（契约 §10.3 与 §10.5）；**§11.1 是本文对契约 §1.1 的措辞修订建议**；**§11.7 是对 `05` 的跨模块字段请求**；其余三条是本文独立发现的。

### 11.1 契约 §1.1 的"毫秒级"是一个现状不成立的端到端承诺（**改契约**）

- **冲突双方**：契约 §1.1「收到**毫秒级落盘、可复访的真实结果**」 vs `apps/web/src/state/useWorld.ts:427-449`（`world_event` → 整层重取）+ `apps/server/src/routes/world.ts:702-892`（重取的固定成本清单）+ `apps/server/src/engine/event-bridge.ts:18,550-558`（`file_changed` 的 150ms debounce）。
...
- **冲突双方**：`docs/doc-20-agent工具与互动字段协议.md:103` 写三个入口「再广播演出」 vs 实测——HTTP 路由**显式不发**帧（`apps/server/src/routes/world.ts:1097-1102`，注释逐字：「The HTTP path sends NO presentation frame (§3.3 step 5): the frontend drives the animation from this response; `dice_result` belongs to the tool path.」），只有 agent 工具路径经 `EventBridge` 发（`apps/server/src/engine/event-bridge.ts:279-298`）。
- **谁对：代码对，文档错。** 三条证据：
  1. `world.ts:1097-1102` 的注释是**有意写的**（含 §3.3 step 5 的交叉引用），不是遗漏。
  2. 玩家路径的前端**只认响应**（`DiceRoller.tsx:58-72`），它根本没有消费 `dice_result` 帧的分支。若 HTTP 也发帧，玩家会看到**两次**仪式。
  3. 前端去重置不住这种双发：HTTP 的指纹前缀是 `playerRequestKey(path)`（`DiceRoller.tsx:30-32`），帧路径的指纹前缀是 `${source}:${identity}`（`dice-ceremony.ts:231`）——`source` 不同则指纹必然不同，`shouldPlayFrame`（`:246-255`）的 `SEEN_TTL_MS` 去重**只按指纹与 path**，也拦不住；而两条入口是**两个独立 listener**（`DiceRoller.tsx:71` 与 `App.tsx:301-320`）。
...
| 作家工具（`roll_dice`） | 同上 | `dice_result` 帧（`event-bridge.ts:279-298`） |
...
- **现状**：`diceStageDisplay`（`apps/web/src/lib/d10-display.ts:19-50`）专门支持 `2d10`、`1~2d6`，其余回落 `d10Display`（`:52-62`，只接受 `count === 1 && faces ∈ {10, 100}`）；引擎的 `parseDiceType` 支持 `count 1..20`、`faces 2..1000`、`modifier ±10000`（`packages/shared/src/rules/dice.ts:44-72`）。
...
| **B. 修 fallback 的诚实性**（不扩支持集） | `rollingFace`（`dice-ceremony.ts:277-280`）的 `ceiling = value > 6 ? 100 : 6` 改成由 `parseDiceType().faces` 决定，tiles 路径即可显示任意骰型的**合法**骰面。约 3 行改动 | **采纳** |
...
- **事实**：`1d100+10` 掷出 `rolls = [40]` 时，权威 `result = 50`（`packages/shared/src/rules/dice.ts:243-256` 的 `sum + modifier`），大数字显示 `50`（`DiceCeremony.tsx:151` 的 `input.result`），但 D10Stage 的 reading 显示 `40`——`d10Display` 逐字只用 `rolls[0]`：`reading = \`${tens} + ${value % 10} → ${value}\``（`d10-display.ts:73-75`），**modifier 不参与**。
- **为什么现在重要**：`roll_dice.expect` 用的是**含 modifier 的 result**（`roll-dice.ts:176` → `evaluateExpect(ast.value, result)`）。命令把"这一掷落在哪一档"变成发奖依据之后，玩家会**盯着骰面读档位**。骰面读法与档位依据不一致，就是"我读出来 40，系统按 50 发奖"。
- **建议**：把 reading 改成显式含调整值，例如 `40 + 10 → 50`。这**不违反** `d10Display` 的定位（`:15-18` 注释："The supplied rolls are authoritative; this function only maps them to display faces and never computes a result"）——它只是把 `parseDiceType` 已经返回的 `modifier`（`dice.ts:44-72` 的 `spec.value.modifier`）从**隐藏**变成**显示**，仍然不重算 result。
...
1. `docs/tools/12:580` 的 `chalk_landed` 载荷只写 `{source,path?,timestamp}`，漏列了实现里必需/使用的 `toolCallId`（`event-bridge.ts:255-266` 实际带它，前端 `useWorld.ts:573-578` 要求它是 string）。
2. `docs/tools/12:570-571` 把 `connected` 的发射点写作 `index.ts:78`，实际在 `index.ts:122`。
...
### 12.1 待拍板项（契约 §11 的编号，本文 MUST NOT 写成既定事实）

| 编号 | 与本篇的关系 | `06` 的立场 |
|---|---|---|
| `[C-1]` 命令执行失败是否落账 | **直接影响**：若不落事件，"这一次失败了"在事件流里不存在，前端就只能靠当前响应的回执 + 卡片上的 `command_error` 表达 | **`06` 需要的最小集是"不落事件也能可见"**：`command_error` 是 frontmatter 写回（不是事件），所以 `[C-1]` 取任何一支，本文的失败可见性设计都不变。方案与代价归 `05` |
| `[C-2]` 命令文件创建是否落 `entity_created` | **间接**：若不落，`command/*.yaml` 不进事件流，前端也无演出（本来就没有）。**注意一个前端后果**：`resolveLayer` 对 `command/` 之外返回 `null`——虽然那是 `world/command/` 的坑（契约 §3.1），但若某天有人把命令放进层里，前端会把它当一张卡渲染出来 | 不表态（归 `07`）；本文只登记"命令文件 MUST NOT 出现在任何层的 items 里" |
| `[C-3]` 角色 Agent 能否创建命令 | **间接**：影响"谁写的"归属，不影响渲染 | 不表态（归 `07`） |
| `[C-4]` 是否需要"世界命令开关" | **直接影响**：硬门 7 的可验证形态就是本文的 §10.5（`status: 'skipped'` + `code: 'command_disabled'`）。**没有开关就没有这条验收** | 不表态（归 `09`）；但要求：**开关关闭时，`commands[0].status` MUST 是 `'skipped'` 而不是"没有 `commands` 键"**——前者玩家被告知，后者是静默 |
| `[C-5]` 36 个既有文件怎么迁 | **间接**：影响前端会不会同时遇到 `dice_outcomes`（旧）与 `on.roll`（新）两种声明 | 不表态（归 `08`）；要求：**并存期内，同一次掷骰 MUST NOT 同时产生"旧路径的后果"和"新路径的后果"**（那就是重复发奖） |
...
4. **`Command_invalid` 的玩家文案是否需要"找作者"的下一步。** `_shared/failure-and-revisit.md:15` 要求失败给出真实的下一步。这里玩家能做的其实只有"继续玩"（作家会叙述）。**若 `[C-4]` 的开关落地，"去找设置打开它"可以成为下一步**；在此之前文案停在"你的判定仍然算数"。
...
| 事实 | 位置 |
|---|---|
| 玩家掷骰：POST + 响应守卫 + `ingestPlayerRoll` | `apps/web/src/components/narrative/DiceRoller.tsx:44-80` |
...
| `HIGHLIGHT_MS=800` / `STILL_ROLL_MS=180` / `FACE_SPIN_MS=90` | `apps/web/src/components/performance/DiceCeremony.tsx:25-29` |
| `settled` 分组的渲染位置 | `apps/web/src/components/performance/DiceCeremony.tsx:140-160` |
| 严格 details 守卫（12 键白名单 + 重建） | `apps/web/src/lib/dice-ceremony.ts:54,65-90` |
| `stageCeremony` 的 legacy 重建 | `apps/web/src/lib/dice-ceremony.ts:281-300` |
| `rollingFace` 的滚动上限 | `apps/web/src/lib/dice-ceremony.ts:277-280` |
| 骰型支持集与回落 | `apps/web/src/lib/d10-display.ts:19-50,52-77` |
| 四态与码集 | `apps/web/src/lib/action-feedback.ts:3,36-44,99-125` |
| `world_event` / `file_changed` 的重取 | `apps/web/src/state/useWorld.ts:424-425,427-449` |
| 幻影排座与过户 | `apps/web/src/state/useWorld.ts:188-190`；`apps/web/src/lib/phantom.ts:20-24,185-190` |
| 新卡以 path 为 key（稳定挂载） | `apps/web/src/components/canvas/Canvas.tsx:544-547` |
| toast 3 秒 | `apps/web/src/App.tsx:219-226` |
| 骰帧 listener（层过滤 / 去重 / 清场） | `apps/web/src/App.tsx:300-326` |
| 仪式 mount（`key={ceremony.key}` + legacy `verdict` 别名） | `apps/web/src/App.tsx:949-951` |
| 奖励信的折叠信封 | `apps/web/src/components/canvas/CardRenderer.tsx:113-123`；`apps/web/src/components/narrative/declared-actions.css:180-213` |
| widget 渲染序列 | `apps/web/src/lib/fm.tsx:26-177` |
| i18n（key = en） | `apps/web/src/lib/i18n.ts:10-31` |
| HTTP 路由不发帧（逐字注释） | `apps/server/src/routes/world.ts:1097-1102` |
| `reply` 摊平 details | `apps/server/src/routes/world.ts:74-80` |
| `/api/layer` 的全部成本 | `apps/server/src/routes/world.ts:702-892` |
| `dice_result` 帧的 11 个字段 | `apps/server/src/engine/event-bridge.ts:284-298` |
| 尾读 drain 与 1s 兜底 timer | `apps/server/src/engine/event-bridge.ts:16,430-462,500-503` |
| `file_changed` 的 150ms debounce | `apps/server/src/engine/event-bridge.ts:18,550-558` |
...
| 注入排除自己（`excludeActor`） | `packages/shared/src/inject/collect.ts:250-252`；`packages/shared/src/store/local-store.ts:745-748` |
| `ActionResult` / `details` 开放形状 | `packages/shared/src/actions/types.ts:28,31-40` |
| 15 个封闭事件类型 | `packages/shared/src/schemas/events.ts:9-25` |
| `parseDiceType` 的支持范围 | `packages/shared/src/rules/dice.ts:44-72` |
| 布局红线（不回写坐标 / 不读 offsetHeight） | `AGENTS.md:103-104` |

---

## 评审回写

（本节留给评审阶段逐条记录：改了哪几处、对应哪条 finding、还剩什么未决。**只改本篇**；`00` 与兄弟篇不动。）
