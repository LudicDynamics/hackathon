...
>
> ⚠️ **快照说明（契约 §R.3 末行）**：全文的 `file:line` 取自本批次开工时的树。**`useWorld.ts` / `App.tsx` / `event-bridge.ts` / `routes/world.ts` 在此之后历经过至少一次提交**（`2e709da feat(ux): wire scene depth and chrome lifecycle`），行号已系统性漂移；这四份代码文件**今天没有未提交改动**（`git status` 干净），本批次的改动**只在 `docs/` 内**。附录给出实测的新行号；**正文中的旧行号按"语义定位"读取，实现前 MUST grep 函数名，MUST NOT 照抄行号**。
...
**为什么顶层摊平而不是嵌套**：`reply` 已经做 `res.json({ ok: true, ...(await run()).details })`（`apps/server/src/routes/world.ts:75-80`），摊平即零路由改动；而 `parseRollDiceDetails` 对 details 是白名单重建（§8.2 红线 2），摊平让"多出来的键"一眼可见。

### 2.2 持久回执：实体 `.md` frontmatter 的 `command_log` / `command_error`

契约 §3.3 冻结：这两个 key 写在**被触发的实体**上，且它们**不随命令文件形态变更**（命令文件已改为 `command/<id>.yaml`，实体仍是 `.md` + frontmatter）。它们是文件的真相，也是玩家复访时唯一还看得见的东西——toast 3 秒后消失（`apps/web/src/App.tsx:274-281`，旧标 `:219-226`）。
...
| 2 | 请求发出 | `world.ts:1156-1174`（`POST /dice`；旧标 `:1102-1119`） | — |
| 3 | 命令在**同一个请求内**跑完 | 触发点（`02`）；契约 §4.3 | 若改成"响应先回、命令异步跑"，玩家看到骰子结果却看不到后果，且没有任何一次返回能证明后果发生过（违反硬门 1 的可复访要求） |
| 4 | 响应/工具返回值带回 `details.commands` | §2.1；`reply` 摊平（`world.ts:80`） | 回执只能靠 `world_event` 补 —— 落到 §3.3 的坑里：慢，而且剧透 |
...
| 7 | **`file_changed` / `world_event` 到达，被 reveal gate 挡住** | NEW，`useWorld.ts:487-489,490-514`（§8.3；旧标 `:424-425,427-449`） | **剧透**：奖励卡在骰子还在转的时候就出现在模糊背景后面（§3.4 算出 150ms 与 10~50ms 两个到达时刻，都远早于 1200ms） |
...
| 11 | `releaseReveal()` → **一次**合并重取 | NEW，§8.3 | 不合并就有 1 + N 个请求排队（`file_changed` 一次 + N 条 `world_event` 各一次）。`reqSeqRef` 只保留最后一个（`useWorld.ts:208-231`，旧标 `:171-175`），但**每一次都在服务端跑一遍** `seatUnplaced`/`reseatLayer` 两个写事务 |
| 12 | 新奖励卡入场 | 现有 `Canvas.tsx:593`（`key={item.path}` 稳定，旧标 `:544-547`）+ 一条 CSS 入场动画 | 无动画则"啪"地出现；动画若依赖测量（读 `offsetHeight`）则违反 `AGENTS.md:104` |
...
`apps/server/src/routes/world.ts:723-918` 一次调用要（**全部是必做的，不是浪费**；下表左栏是 §3.3 写就时的行号，该文件已被 `2c1684b` 改动，**按右栏的符号定位**）：
...
- 新奖励卡的位置**由服务端排座给出**（`seatUnplaced`，`world.ts:802`；旧标 `:781`），前端 MUST NOT 为了"让新卡落在好看的格子"而回写坐标。`AGENTS.md:103` 的红线逐字：「`Canvas` 的取景 effect 只能移动相机，不能把首帧排版结果回写卡片坐标；否则首次拖拽可能让整层跳位。」
...
- 失败卡的 `command_error` **默认展开**会让该卡一进来就比同类卡高。这是可接受的：`POST /api/card/footprint`（`useWorld.ts:410-419`，旧标 `:347-355`）本来就为"把实测高度写回"而存在，而 `reseatLayer` 只为"声明尺寸或实测尺寸**合法漂移**了的卡"重排（`world.ts:807-813`，旧标 `:785-790`）。但这意味着**失败卡的入场动画更不能用高度驱动**。
...
**关于 turn 的一条修订（契约 §10.7 已证伪原表述）**：`serviceFor` 的 `randomUUID()` 在**函数体内**（`apps/server/src/routes/world.ts:63-65`），所以**每调用一次就铸一个新 turn**。`POST /api/choice` 先 `runDeclaredChoice(serviceFor(...))`，未命中再 `serviceFor(...).chooseOption(...)`——**两次** `serviceFor` → **一次 HTTP 请求产生两个 turn**（`world.ts` 的行号随 `2c1684b` 漂移，按语义定位 `serviceFor` 的两次调用）。
...
**前端可以从事件表里拿什么、不能拿什么**：`world_event` 帧带完整 `WorldEvent`，前端**只用来决定"重取"**（`useWorld.ts:490-514`，旧标 `:427-449`）。`roll_resolved` 不进演出入口（`useWorld.ts:741` 的 `default` 分支明确忽略历史事件）——**这条必须保持**，否则复访一个世界会把历史骰子重新演一遍。
...
**但立一条硬约束**：`useWorld` 的 `case 'file_changed'` 与 `case 'world_event'`（`useWorld.ts:487-489,490-514`，旧标 `:424-425,427-449`）触发重取时 **MUST NOT** 附加任何"新卡提示音 / 闪烁"。否则作家路径会突然冒出玩家从未发起过的演出反馈。
...
| `dice_result` 帧 | `useWorld.ts:670-673` → `App.tsx:426-460`（旧标 `:584-587` / `:300-326`） | 帧里多一个可选 `commands`；`markPlayed`/去重逻辑**不变** |
| `world_event` | `useWorld.ts:490-514`（旧标 `:427-449`） | fetchLayer 改走 reveal gate（§8.3） |
| `file_changed` | `useWorld.ts:487-489`（旧标 `:424-425`） | 同上（**这条最容易漏，它比 `world_event` 先到**） |
...
`code` 的最终命名**分两层**：**触发层**归 `02`（`CommandOutcomeCode`，16 个），**效果层**归 `04`/`03`（`WorldCommandRuntimeErrorCode`，`03` 拥有）；下表是 `06` 对**可见性**的要求，不是码集的拥有者。**本表同时是 `action-feedback.ts` 的改动输入**：`CONFLICT_CODES`（`apps/web/src/lib/action-feedback.ts:36-40`）与 `REJECTED_CODES`（`:41-44`）今天都不含这些码，落不进去就会一律兜底成 `failed`（`:108`），玩家看到红色"执行失败"而实际只是"世界拒绝"。**该文件已漂移**：现在的兜底分支在 `:103-108`（`classifyActionResult` 在 `:99`）。
...
`parseRollDiceDetails` 是**严格**的（`dice-ceremony.ts:54,65-90`）：12 个键必须齐、类型必须对，否则返回 `null`，玩家看到 `DiceRoller.tsx:68` 的 `missing authoritative roll details`。这个严格是对的——**骰子细节错了，整个仪式没有意义**（没有 `rolls` 就没有骰面可演）。
...
| 复用 | 位置 | 怎么用 |
|---|---|---|
| `dice-outcome-letter` 的折叠信封样式 | `CardRenderer.tsx:150-157`（旧标 `:118-121`）；`declared-actions.css:181-219` | `command_error` / `command_log` 共用它的排版骨架（padding / border / `max-height` / `<summary>` 折叠），只加一个 tone 变体类（`.dice-outcome-letter--error`）改边框与标题色。**不新建一整套样式表** |
...
- `dice_reward` 是**奖励**，折叠着等玩家点（`CardRenderer.tsx:150-157` 的 `<details>` 无 `open`；旧标 `:118`）。
- 失败必须主动撞到玩家眼里，否则等于静默。**默认展开是硬门 4 在这个组件里的落地形态。**

**为什么必须是卡片而不是 toast**（现状 `App.tsx:274-281`：`setToast` + 3 秒后清空；旧标 `:219-226`）：
...
apps/server/src/engine/event-bridge.ts:327-341               dice_result 帧加 commands 字段（旧标 `:284-298`）
```

`dice_result` 帧的改动（在既有 push 对象里加一段）：

```ts
// apps/server/src/engine/event-bridge.ts:327-341 —— 只加字段，不新增帧（旧标 :284-298）
push({
  type: 'dice_result',
  source,
  path: details.path,
...
**路由零改动**：`reply` 把 `details` 摊平（`world.ts:80`；旧标 `:79`），`commands` 自动进响应体。
...
> ⚠️ **本节行号同样受顶部的快照说明约束**：`useWorld.ts` / `App.tsx` / `event-bridge.ts` 已漂移（上表 `useWorld.ts` 两行已给实测值；`App.tsx:950` 的 `verdict={ceremony.verdict}` 现在是 `:1169`，实现时按语义定位）；其余文件未动。
...
> - **会**产生 `file_changed`——它写的 `.airpworld/canvas.db`（`packages/shared/src/store/local-store.ts:122`）不在 watcher 的忽略名单里（只忽略 `.airpworld/assets/`、`.airpworld/sessions/`、`node_modules`，`event-bridge.ts:579-583`）。
>
...
```ts
// apps/web/src/state/useWorld.ts:487-489 —— file_changed（先到，最容易漏；旧标 :424-425）
case 'file_changed':
  // 命令/骰子落盘会在 ~150ms 触发这条；仪式期间压住，收场后重取一次。
  deferReveal(() => { void fetchLayer(layerRef.current); });
  break;

// apps/web/src/state/useWorld.ts:490-514 —— world_event（旧标 :448）
...
**gate 的代价（如实登记）**：仪式播放期间世界视图冻结最多 2.5 秒。这**不是新增的损失**——全屏模态遮罩（`DiceCeremony.tsx:102-107` 的 `fixed inset-0 z-50`）本来就把画布挡得只剩一坨模糊背景，此时更新与否玩家都看不清。切层会立即收场（`App.tsx:200-201` 的 `clearAdmittedCeremony` → `clearCeremony()`），gate 随 `releaseReveal` 放行，不会有跨层残留。
...
| # | 现状 | 位置 | 本批目标 | 不改的代价 |
|---|---|---|---|---|
| 1 | 玩家掷骰的可见后果**只能**来自整层重取 | `useWorld.ts:487-489,512`（旧标 `:424-425,448`） | 回执搭同步响应；重取被 gate 压后到仪式收场 | 契约 §1.1 的"毫秒级"在玩家侧不成立（§3.3） |
| 2 | `details` 守卫是**白名单重建**，额外字段静默丢弃 | `dice-ceremony.ts:54,65-90` | 显式吸收可选 `commands`；`commands` 不进必填键列表 | 服务端加了字段也到不了 UI，且**没有任何报错** |
| 3 | `dice_result` 帧搬 11 个字段，不搬 `forged`（有意） | `event-bridge.ts:327-341`（旧标 `:284-298`） | 加搬一个 `commands`（可选、有意，写进文档） | 作家/角色触发的命令对玩家侧无回执 |
| 4 | 失败只能靠 toast（3 秒消失） | `App.tsx:274-281` | 卡片常驻 `command_error`，默认展开 | 硬门 4 不成立（§7.5） |
| 5 | 前端只有四态，码集不含命令码 | `action-feedback.ts:36-44` | 命令码进 CONFLICT/REJECTED 集，**不新增第五态** | 一律兜底 `failed`，玩家看到误导性的红色"执行失败" |
| 6 | 工具路径的后果**看不见**（`excludeActor` 把自己过滤掉） | `inject/collect.ts:105,111,126,251`；`local-store.ts:772` | 后果走 `details` → tool_result（§6.2a） | 作家重复发奖 |
...
| 9 | `1d100+10` 的 D10Stage reading 显示 `40`、大数字显示 `50` | `d10-display.ts:53-77` vs `DiceCeremony.tsx:151` | 见 §11.4 | 玩家盯着骰面读档位，与发奖依据不一致 |
...
1. 掷一次，看到回执与奖励卡；刷新页面。
2. 断言：**没有**骰子仪式（`roll_resolved` 不进演出入口，`useWorld.ts:741` 的 `default` 分支；旧标 `:654-656`）；`command_log` 仍在卡片上；奖励卡仍在。
...
- **冲突双方**：契约 §1.1「收到**毫秒级落盘、可复访的真实结果**」 vs `apps/web/src/state/useWorld.ts:490-514`（`world_event` → 整层重取，旧标 `:427-449`）+ `apps/server/src/routes/world.ts:723-918`（重取的固定成本清单，旧标 `:702-892`）+ `apps/server/src/engine/event-bridge.ts:18,593-600`（`file_changed` 的 150ms debounce，旧标 `:550-558`）。
...
- **冲突双方**：`docs/doc-20-agent工具与互动字段协议.md:103` 写三个入口「再广播演出」 vs 实测——HTTP 路由**显式不发**帧（`apps/server/src/routes/world.ts:1154-1155`，注释逐字：「The HTTP path sends NO presentation frame (§3.3 step 5): the frontend drives the animation from this response; `dice_result` belongs to the tool path.」），只有 agent 工具路径经 `EventBridge` 发（`apps/server/src/engine/event-bridge.ts:322-341`）。
- **谁对：代码对，文档错。** 三条证据：
  1. `world.ts:1154-1155` 的注释是**有意写的**（含 §3.3 step 5 的交叉引用），不是遗漏。
  2. 玩家路径的前端**只认响应**（`DiceRoller.tsx:53-79`），它根本没有消费 `dice_result` 帧的分支。若 HTTP 也发帧，玩家会看到**两次**仪式。
  3. 前端去重置不住这种双发：HTTP 的指纹前缀是 `playerRequestKey(path)`（`DiceRoller.tsx:30-32`），帧路径的指纹前缀是 `${source}:${identity}`（`dice-ceremony.ts:231`）——`source` 不同则指纹必然不同，`shouldPlayFrame`（`:246-255`）的 `SEEN_TTL_MS` 去重**只按指纹与 path**，也拦不住；而两条入口是**两个独立 listener**（`DiceRoller.tsx:71` 的 `ingestPlayerRoll` 与 `App.tsx:426-460` 的 `onDiceFrame`）。
...
| 作家工具（`roll_dice`） | 同上 | `dice_result` 帧（`event-bridge.ts:322-341`） |
...
- **事实**：`1d100+10` 掷出 `rolls = [40]` 时，权威 `result = 50`（`packages/shared/src/rules/dice.ts:254` 的 `sum + spec.modifier`；旧标 `:243-256`），大数字显示 `50`（`DiceCeremony.tsx:151` 的 `input.result`），但 D10Stage 的 reading 显示 `40`——`d10Display` 逐字只用 `rolls[0]`：`reading = \`${tens} + ${value % 10} → ${value}\``（`d10-display.ts:73-75`），**modifier 不参与**。
...
1. `docs/tools/12:580` 的 `chalk_landed` 载荷只写 `{source,path?,timestamp}`，漏列了实现里必需/使用的 `toolCallId`（`event-bridge.ts:299` 实际带它，前端 `useWorld.ts:659-661` 要求它是 string）。
...
| `world_event` / `file_changed` 的重取 | `apps/web/src/state/useWorld.ts:487-515`（旧标 `:424-425,427-449`，**快照已漂移**） |
| 幻影排座与过户 | `apps/web/src/state/useWorld.ts:227`（`reconcileLanded`，旧标 `:188-190`）；`apps/web/src/lib/phantom.ts:20-32,208` |
| 新卡以 path 为 key（稳定挂载） | `apps/web/src/components/canvas/Canvas.tsx:593`（旧标 `:544-547`，**已漂移**） |
| toast 3 秒 | `apps/web/src/App.tsx:274-281`（旧标 `:219-226`，**快照已漂移**） |
| 骰帧 listener（层过滤 / 去重 / 清场） | `apps/web/src/App.tsx:426-460`（旧标 `:300-326`，**快照已漂移**） |
| 仪式 mount（`key={ceremony.key}` + legacy `verdict` 别名） | `apps/web/src/App.tsx:1169`（旧标 `:949-951`，**快照已漂移**） |
| 奖励信的折叠信封 | `apps/web/src/components/canvas/CardRenderer.tsx:150-157`（旧标 `:113-123`，**已漂移**）；`apps/web/src/components/narrative/declared-actions.css:181-219` |
| widget 渲染序列 | `apps/web/src/lib/fm.tsx:26-174`（`renderFrontmatterWidgets`） |
| i18n（key = en） | `apps/web/src/lib/i18n.ts:10-31` |
| HTTP 路由不发帧（逐字注释） | `apps/server/src/routes/world.ts:1154-1155`（旧标 `:1097-1102`，**已漂移**） |
| `reply` 摊平 details | `apps/server/src/routes/world.ts:75-80`（旧标 `:74-80`） |
| `/api/layer` 的全部成本 | `apps/server/src/routes/world.ts:723-918`（旧标 `:702-892`，**已漂移**） |
| `dice_result` 帧的 11 个字段 | `apps/server/src/engine/event-bridge.ts:327-341`（旧标 `:284-298`，**快照已漂移**） |
| 尾读 drain 与 1s 兜底 timer | `apps/server/src/engine/event-bridge.ts:16,475,544-546` |
| `file_changed` 的 150ms debounce 与 watcher 忽略名单 | `apps/server/src/engine/event-bridge.ts:18,580-581,590-600`（旧标 `:18,550-558`，**快照已漂移**） |
...
| 注入排除自己（`excludeActor`） | `packages/shared/src/inject/collect.ts:105,111,126,251`；`packages/shared/src/store/local-store.ts:772` |
