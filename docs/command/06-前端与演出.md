...
`code` 的最终命名**分两层**：**触发层**归 `02`（`CommandOutcomeCode`，16 个），**效果层**归 `04`（**唯一定义位：`04 §6.3.2`** 的 6 个 `command_*` 可见性标签 + 它们各自 `derived from` 的线上值）；下表是 `06` 对**可见性**的要求，不是码集的拥有者，**MUST NOT 在此重述语义**（契约 §11.3 C7 / C5 的"一处实现 + 其余引用"）。**本表同时是 `action-feedback.ts` 的改动输入**（2026-09-15 实测行号）：`CONFLICT_CODES` 在 `apps/web/src/lib/action-feedback.ts:100-113`、`REJECTED_CODES` 在 `:114-119`、`FAILED_CODES` 在 `:120-124`，`classifyActionResult` 在 `:204`，兜底分支在 `:214`。**该文件今天已含这些码**（`command_reused` / `command_target_missing` / `command_limit_exceeded` 在 `CONFLICT_CODES`、`command_not_allowed` 在 `REJECTED_CODES`、`command_effect_failed` / `command_write_failed` 在 `FAILED_CODES`），因此不会再兜底成 `failed`。

| code（可见性标签，语义与触发条件见 `04 §6.3.2`） | status | 进哪个码集 | 模型看到的文案（`ActionResult.text` / tool error，**英文**） | 玩家看到的文案（i18n，**英文 key**） |
|---|---|---|---|---|
...
> **命名对齐（`[C-4]` 评审 §7 第 5 条，`02` 是触发层码集的拥有者；2026-09-15 按 C7 收口）**：本文此前用的 `command_missing` / `command_invalid` **不是 `02` 产生的码**——`02 §10.2.1` 的 16 个 `CommandOutcomeCode` 里对应的是 **`command_not_found`** 与 **`command_malformed`**，所以上两行已改。**本节其余 6 个 `command_*` 码的唯一定义位在 `04 §6.3.2`**——它们是**效果层的可见性标签**（不是 `ActionErrorCode`，也不在 `CommandOutcomeCode` 里），每个都 `derived from` 一个已落地的线上值（`not_found` / `write_failed` / `effect_failed` / `CommandSkipReason.already_done` / 上限码族）。本文只声明**可见性要求**（两张文案 + 前端四态），**MUST NOT** 替它们重述定义；`04 §6.3.2` 是那个唯一的地方（契约 §R.4 的"符号所有权"族 + §11.3 C7）。
...
> **这些是可见性标签，不是线上值**：下面断言里的 `command_*` 是 `04 §6.3.2` 定义的**前端分桶标签**（`action-feedback.ts` 的 `CONFLICT_CODES` / `REJECTED_CODES` / `FAILED_CODES` 键），`classifyActionResult` 收的正是标签；线上回执里的取值是它们各自的 `derived from`（`not_found` / `effect_failed` / `already_done` …）。这条测试断言的是**标签 → 四态的映射**，不是"线上会发这些字符串"。
...
3. **两套码之间的映射**（哪个已落地的线上值对应哪个可见性标签）**已由 `04 §6.3.2`（C7 收口）声明**：那 6 个 `command_*` 是**可见性标签**，每个都给出 `derived from`（`not_found` / `effect_failed` / `write_failed` / `CommandSkipReason.already_done` / `WorldCommandRuntimeErrorCode` 上限族）。`ActionErrorCode` ↔ `WorldCommandRuntimeErrorCode` 之间的完整对应仍归 `05`（拥有失败口径）+ `04`（拥有效果集）在实现前给出；`06` 的渲染纪律是"两处各查各的、不互相翻译"（§2.2 的注），所以**前端不因此阻塞**。
