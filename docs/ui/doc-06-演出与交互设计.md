...
2026-09-13 选项确认流：桌面 / 小天地选项点击只准备行动，跨场景时先走原有门禁进入（不触发初始化），成功后把选项原文填入作家输入框、展开聚焦，并显示“已选择 / 待发送”的具体描述。未发送前不调用 choose，不落选择事件、不启动作家；发送等价于玩家在作家栏输入，附带源实体路径供读取。前端不再把“已填入”冒充“世界正在响应”。

桌面角色使用窄竖幅动画立绘与轻边框；一个画布最多播放一个人物视频，优先 hover 角色，关闭特效 / 减少动态 / 页面隐藏时静态回退。桌面实体行动区、右侧常驻角色 hover 与对话框均提供小天地入口；只有真正空的小天地显示显式初始化按钮，不覆盖已有内容。小天地入口与空态支持中英日文。

作家阶段信息在右上角三行区域更新，最终回执仍淡出。物品创建 / 移动提示只消费已落账 world_event，排除 Chalk 创建、编辑和失败事件；按事件 id 去重、800ms 合并、两次展示至少间隔 4.5s，不根据工具开始猜测成功。

作家每轮最终公开回执由 WriterResult 显示在右上角场景状态栏下方，最多三行，超出省略；每轮替换一条，7 秒后消失，随 chrome 一起隐藏。不把回执放在画布中央或底部，不截取固定字符数代替三行排版。世界文件中的正式 Chalk 仍按实体路径渲染，不受此回执布局影响。

当前存档被外部删除后，前端收到 `409 no_active_world` 清除旧画布、角色和背包，打开世界选择。世界列表单独加载，不因正文接口失败而消失。不自动恢复已删除存档，也不自动进入可修改模板的模式。

niko 停止入口：作家运行期间右下角独立显示 Stop writing，不依赖输入栏展开或 chrome 显隐。点击仅发送 `writer_abort` 并显示 Stop requested，实际结束依赖后端状态，不能先行伪装 idle；保留重试入口。该按钮尚不保证取消排队任务或初始化子代理。

niko 作家反馈：不投影每次 Chalk 工具调用的临时正文；每轮 `writer_idle` 后只在右上角状态栏下方显示最后一条公开 `writer_message`（最多三行，超出省略），约 7 秒自然淡出。同屏最多一条，新轮开始或切场景即清除，不展示 reasoning/thinking 或工具参数。无公开回复时仅提示行动处理结束，不宣称已落板。真实 Markdown 板书仍保留在画布与文件系统中。

niko 展示约定：删除独立固定 `SceneChalk` 面板（含小天地共用 Canvas 的挂载），场内正文仅通过 `.object > .chalk--bare` 和 `EntityInteractions` 展示。README 的后端 `scene` 数据保留，仍用于场景配置与小天地元信息，不自动重复展示正文。只有 README 的旧场景需要将玩家可读正文/选项建成场内实体，不能通过恢复固定面板补齐。
...
Agents 面板与行动入口展示实际模型、公开执行阶段、耗时；Chalk 落板与整轮完成分开。切换模型按世界存档保存，忙碌中禁止切换，不清空角色记忆。详《agent/Agent模型选择与进度》。

循环媒体（2026-09-13）：已匹配的 Seedance 背景和透明人物接入所有对应模板与存档，覆盖及缺口见《frontend/动态素材接线》。特效开启才播放当前背景和打开的角色特写；小头像仍用透明静图。立绘 contain 保留完整轮廓，不可见或页面隐藏暂停；关闭特效卸载视频，错误回退静图。BGM 与静音视频独立。

背包交互补齐（2026-09-13）：点击背包物品打开完整 Markdown 与互动字段，可选择「放回当前场景」复用 `/api/move`；只在成功后关闭，遇到同名冲突时留在背包并显示错误。拖拽保留。

作家/角色实际接线（2026-09-13）：作家显示工作与停止状态；角色只渲染带本角色 ID 的真实增量/最终回复，不再随机代答。错误显式显示。数据路径与帧字段见《agent/Agent前端接线》。

未写之门 Demo（2026-09-12）：信封/手机可用 `visual` 声明道具外形，门通过 target 进入空目录。查看外观与选择使用分离；choice 接受后自动派发作家。阅读对话框不撑大画布实体，图片缺失时显示 CSS 道具。具体范围见 gameplay/doc-25。
...
当前设计批次的唯一排版口径见 [`docs/layout/00-共同上下文.md`](layout/00-共同上下文.md)：共享 `flowColumns` 按稳定顺序从上到下填一列，超过列高后向右开列；候选列遇到旧卡继续向右扫描。服务端真实卡、生成幻影与 footprint 漂移共用该几何，真实 footprint 仍以 `cards.width/height` 为准。

幻影座位=成品座位（座位过户纪律）；落地帧不重新排座，真实 `/layer` 返回的坐标优先。非上帝模式下 Chalk 可阅读和互动，但不进入卡片拖拽；上帝之手显式开启独立编辑状态后才可拖动。
...
日语迁移补充：角色显示名使用 manifest 可选 `characters[].name`，协议仍发送 `id`。日语世界载入时切日语 UI；旧遮罩尚未消费真实回复帧，日语模式只显示明确的接线未完成状态，不播放英文随机台词，也不把状态提示当人物回应。日文输入法组合期间 Enter 不发送。范围见《worlds/世界日语化迁移》。
...
## 5. 右侧栏（上下布局：角色栏在上、背包在下）

> 玩家小天地（`player/` 目录）与角色导航的前端呈现。**两者都投影文件系统**：背包 = `player/` 目录列表，角色栏 = `characters/` 目录扫描 + `presence` 表的在场状态。与角色小天地（可进入逛的空间）不同——**右侧栏是常驻的列表预览**，资源管理器式。
>
> **⚠️ 布局修订（2026-09-14，「角色在场与跟随」批次 P-5）**：本节初稿画的是**左右双 tab 侧边栏**（切走画布右侧一条）。实际外壳是**沉浸式**的，右侧是**上下两块常驻悬浮栏**：角色栏在上、背包在下，没有 tab、不占画布宽度。以本节下方的新布局为准。
...
- **角色栏在上**：`characters/` 的每个角色一「行」（头像 + 名字 + 状态 + 功能键），
  在场状态来自 `presence` 表；交互与三档状态见 §5.4 与
  `docs/presence/00-共同上下文.md` §2.2/§4.2.1；
- **背包在下**：`player/` 目录的实时列表，新增/删除/重命名即反映在文件系统
  （路径即 id，无额外索引）；
...
- **背包物品详情的两个入口**：`Place in current scene`（日语：今のシーンに置く）仍执行放置；`Use this item`（日语：このものを使う）只向作家输入框追加可编辑的使用意图，不移动、不消耗、不提前发送。可连续加入两件及更多物品，同一路径重复点击去重，不覆盖已有 RP 或选项文字。
- **准确引用，不靠显示名猜物品**：草稿中显示 `[本地化标题](稳定文件路径)`；发送时从最终文本提取仍在背包中的准确路径，附带读取物品与当前场景 README 的要求。删除引用就不再附带该物品；编辑成“不使用”时以最终意图为准。切换世界清空草稿，不把旧世界物品带到新世界。
- **使用不等于放行**：作家按最终意图与场景规则判断出示、组合、提交或使用；用途或目标不清楚时通过 Chalk 追问。合法行动必须完成必要的物品／状态落盘并给出结果和下一步，不能只回应旁白。沿用现有 `writer_prompt` 通道，不新增门禁绕过或自动消耗逻辑；具体组合成功仍由世界规则和作家执行决定。
...
### 5.2.1 游玩辅助：下一步提示与执行记录

- 右上角 `Continue · next-step hint` 准备一条可编辑的求助请求，仍由玩家按发送后交给作家；不再默认要求剧情自动前进一步。提示只依据已发现事实和当前背包，给出“在哪里、可以尝试什么、为什么”，不泄露未发现答案、不替玩家解锁或发送传真。只允许写一段提示 Chalk，不改游戏事实。
- `Settings → Play assistance → Show Continue / next-step hints` 默认开启，关闭后按钮完全隐藏；偏好保存在当前浏览器，不改存档与自动续写档位。
- Continue 下方是近期执行记录：每次工具开始／结束、成功或中断状态，以及世界事件确认的已落盘变更。步骤显示安全的世界相对路径，不展示原始参数、模型思考、密钥或整篇文件内容。
- 本轮结束显示工具完成数和已记录变更数，保留可展开的作家最后回应；结束不自动声称“通关成功”。中断后即使收到 idle，也继续标记“未确认完成”，已完成变更不被抹掉。记录仅覆盖当前浏览器接收的最近 80 条；切换场景保留，切换世界清空，不冒充完整服务端审计日志。
...
**角色栏 = 世界角色列表**（`characters/` 目录扫描 + `presence` 表的在场状态）。每个角色一行：
头像 + 名字 + 在场状态；**功能键在展开器/悬停里**，不常驻占位。

> **⚠️ 语义修订（2026-09-14，「角色在场与跟随」批次 P-1）**：初稿是「点头像 = 导航」＋独立「聊」按钮。
> 现改为 **点头像 = 开启直聊**；**导航移入功能键「去 ta 那里」**。理由：画布头像的点击语义已经是
> 「进入对话」（`docs/ux/00` 的「角色头像遵守单击进入对话语义」），侧栏保持一致可少学一套动作语法；
> 而导航是低频、需明确意图的动作，误触会把玩家传送走。完整定义见
> `docs/presence/00-共同上下文.md` §2.2（含触屏/键盘等价路径）与 §7 P-1。

| 能力 | 操作 | 结果 |
|---|---|---|
| **聊天** | 点角色头像（或键盘 Enter/Space） | **仅当同场景**时，在该角色所在场景与他进入**对话遮罩**（等价于画布上单击该角色头像，见 §3）。不在场/在别处时**不可直聊**，只给可见提示 |
| **导航** | 功能键「去 ta 那里」 | 同场景 → 相机移动到角色坐标；在别处 → **跨场景并**移动到 ta 的位置；不在场 → 提示"ta 此刻不在这里"，**不导航**（不硬造位置） |
...
8. **角色栏**：点「去 ta 那里」导航到角色场景（切层+定位）→ 点头像进入遮罩（直聊）→ 点「跟随」后切场景角色跟着来（画布上出现该角色头像，带阻尼入场）。
...
12. 角色栏的"不在场"态导航提示（"ta 此刻不在这里"）够不够？要不要显示角色最后出现的场景位置？（先只做提示，位置留待叙事感知。）
13. **骰子演出特效的节奏**（纸骰弹跳/旋转 → 落定的时长/帧/缓动，"展开本身就是内容"参照 doc-04 §6 登录墙节奏）——并入 **doc-09 #4**。

## 阅读与音效细化

普通文档在画布原位置展开为约 500px 纸片，保留倾角，不出现全屏遮罩；背包使用左下角小纸片。查看不触发剧情行动，点击标题、折回按钮、纸外或 Escape 收起。世界选择使用无背景遮罩的侧边纸面目录，仍按世界组织存档。交互音复用 Canvas 素材与音量，相同音效不重叠、迟到不补播。完整规则见 [阅读与音效优化](frontend/阅读与音效优化.md)。

## 门禁反馈补充（2026-09-13）

预期的 `requirements_not_met` 是场景内反馈：使用 Gate 的顶层 `blocked` 短旁白、缺失物的本地化标题与短暂描边，允许返回现场或 Escape 收起；不显示 HTTP/JSON，不触发作家或自动骰子。真正的网络、配置与服务故障仍明确报告。实现与迁移边界见 [体验版稳定路径与门禁演出](worlds/体验版稳定路径与门禁演出.md)。
