...
项目 OpenAI-compatible 适配器保留 HTTP 状态、请求尺寸/质量/是否参考图，以及上游 JSON 的 `code/type/param/message`，截断并脱敏；不输出完整响应、请求正文或非 JSON 网关页面。错误沿工具异常通道传递，避免引擎将正常返回的错误对象标成完成。不修改 pi-rp。

moon 已知出现 HTTP 400，但旧记录没有上游错误正文，不能确定是尺寸、提示词还是网关兼容问题。本批保持成功请求的尺寸、模型、质量和 generations/edits 路径不变，不自动重试收费调用。下一次真实失败才可据新诊断定位；适配器假响应回归测试不等于线上生图验收。
