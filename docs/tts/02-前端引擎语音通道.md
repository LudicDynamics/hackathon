...
graph LR
  ...
  B -->|"playClip(master, buf, loop=false, VOICE_SAMPLE_LEVEL)"| N["src → gain → master"]
  N --> SPK["扬声器"]
...
playVoice(url):
  ...
  6. loadSample(url).then(buf => {                          // audio.ts:532，按 URL 缓存解码
       ...
       nodes = playClip(master, buf, /*loop*/ false, VOICE_SAMPLE_LEVEL)
       if (!nodes) return
...
`playClip(master, buf, /*loop*/ false, …)`。漏了（误传 `true`）→ 一句台词无限循环，且 `playClip` 不注册 `onended`（`audio.ts:585`），`voiceClip` 永不清空，`isVoicing()` 永真。
