...
- **透传层不做 token 化**（背景说明，**非**放宽正则的理由）：曾有「音色 ID 含空格」的担忧，`docs/tts/00 §15.1` 已实测**证伪**（`Eldric Sage` 不存在，是两个 ID 拼接）。故透传层只做 `.trim()`（去首尾空白），**不**做字符白名单（校验归 `01`）；`voice` 实际是**单 token**，符合 `docs/tts/00 §4.2` 步骤 4 的 `/^[A-Za-z0-9_-]+$/`（该正则**不变**）。见 §11 冲突 5。
...
5. **[已裁决 §15.1] 音色 ID 含空格的担忧前提为假**：`04` 曾担心含空格 ID（`Eldric Sage`）会被 `/^[A-Za-z0-9_-]+$/` 误拒。`docs/tts/00 §15.1` 实测裁定：`Eldric Sage` **不是一个真实音色**（`Eldric` 与 `Sage` 是两个独立 ID，被拼接）；正则**不变**。故本模块**不**为空格放宽任何东西：`/api/characters` 透传层仍只 `.trim()`（去首尾空白，不 token 化），`voice` 值是**单 token**。§3.2 中「音色 ID 可能含空格」的⚠️按此裁决**降级为背景说明**（真实受限值仍是单 token）。
