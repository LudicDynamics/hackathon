...
stateDiagram-v2
  [*] --> idle
  idle --> mock: 本世界内容语言首次打开（localStorage v2 未记）
...
| `localStorage.setItem('airp:greeted:v2:<worldId>:<charId>:<contentLanguage>', '1')` | 此世界内容语言的 mock 引导语首次显示时，**仅 `worldId` 有值** | 前端存储 |
...
CharacterModal mount（:277-293 复位 effect 之后）：
  ...
  if (greeted === null) {
    const g = GREETING_LINE[language === 'ja' ? 'ja' : 'en']; // 问候和 TTS 都跟世界内容语言
...
- **语言来源（契约 §15.17）**：引导语与沉默台词取世界内容语言 `language`，与 TTS 一致；无 world.json.locale 时回落英文。UI locale 仅影响按钮与界面文案。
...
| TTS 响应 `truncated:true` | 本批**不显示**标记；且 **MUST NOT 重试**（契约 §15.21：重试风暴比截断更糟）——页文本仍原样逐字显示，只是朗读被截短（登记 §12） | — |
...
| 6 | §6.1 | **§15.17**：`GREETING_LINE` 跟世界内容语言，与 TTS 一致；UI locale 只控制界面。 | 契约 §15.17 |
