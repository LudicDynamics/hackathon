...
顶部“声音与设置”提供本浏览器语音开关、服务配置状态、当前模型和默认音色，以及 DashScope TTS、OpenAI-compatible、Flow media proxy 的服务地址和密钥输入。已有密钥不回显，不进入 localStorage；输入框留空保留原值，关闭面板销毁草稿。角色声明的音色优先；模型和默认音色仍只展示。
