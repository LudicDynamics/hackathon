...
> **UX 舞台统一批次接缝（2026-09-13）：** TTS 只拥有语音请求、页语义和 audio playback；角色入口由 UX03 的 App inline Chrome 拥有。本文中 `RightSidebar` 作为旧入口的引用仅是现状/历史，不得阻止或重新引入 UX batch 的 inline Chrome 迁移。
...
6. `docs/doc-06-演出与交互设计.md §3`（遮罩的解剖，本批**实现并修订**它，见 §12）；
...
- `text` **必填**，角色一页的台词。前端和服务端共同调用 `packages/shared/src/rules/tts-text.ts::sanitiseTtsText`：先移除 `[emo: tag]` 与成对 `() / （） / [] / 【】` 动作片段；异常括号 fail-closed 为空。页面显示仍保留原文，TTS 请求只携带清洗后的可朗读文本；清洗后空串 → 前端不请求，服务端收到空值 → 400 `invalid_argument`。
...
```text
hash = sha256(`${model}|${voice}|${language_type}|${text}`).slice(0, 20)   // hex，20 字符
file = `${hash}.wav`
```
...
2. 校验原始 `text`：`typeof === 'string'` 且 `trim() !== ''` → 否则 **400**。
   - 漏了 → 空文本合成出奇怪音频并污染缓存。
3. 调用共享 `sanitiseTtsText`；前端已预检所以通常不会到空值，但服务端必须幂等重清洗 → 空值 **400 `invalid_argument`**。
   - 漏了 → 括号动作会被朗读，或绕过浏览器直接调用 API 时污染厂商与缓存。
4. **截断**：清洗后的 `text.length > 500` → 取前 500 字符并置 `truncated: true`（DashScope 上限 512 token ≈ 600 字符）。
   - 漏了 → 超长请求被 DashScope 拒，整页无声。截断是「保住演出」而非「静默失败」：`truncated` 进响应体，前端可据此标记。
5. 解析 `voice`：请求 `voice` → **调色板解析**（别名 → 商品名；裸 id → 自身；未知/畸形 → 回落 `AIRP_TTS_DEFAULT_VOICE` 并 warn）——**唯一解析点**，见 `07 §3`。形状体检 `VOICE_RE = /^[A-Za-z0-9][A-Za-z0-9 _-]*$/`（**已被 `07 §0` 修正：允许空格**，原 `/^[A-Za-z0-9_-]+$/` 会静默改掉合法音色 `Eldric Sage`）。
   - 漏了 → 畸形 voice 会让 DashScope 400，前端整页无声。
6. 解析 `language_type`（§4.4）。
7. `hash` → 若 `<cacheDir>/<hash>.wav` 已存在 → **直接返回** `{ url, cached: true }`，不联网。
   - 漏了（不查缓存）→ 每次翻页重复合成，延迟与成本飙升。
8. 否则 `synthesise(...)` → 写盘（**原子写**：先写 `.tmp` 再 `rename`，避免半截文件被后续请求当成缓存命中）。
   - 漏了原子写 → 并发/中断时缓存里是半截 wav，前端 `decodeAudioData` 失败且**永不重试**（`failCached`）。
9. 返回 `{ ok:true, url: '/api/tts/audio/<hash>.wav', cached:false, characters: <text.length>, truncated }`。
...
> 本节**修订** `docs/doc-06 §3.2` 的「逐字流式」一格：流式**保留**（页内仍逐字出字），但**演出单位从「整段」改为「一行 = 一页」**，并新增语音与点击推进。
...
2. **仅首次**：用 `localStorage` 记住「这个世界里这个角色已打过招呼」，key = `airp:greeted:<worldId>:<charId>`。
...
不新增 `events` 行、不新增 `type`（封闭十五类，`docs/doc-21`）。

### 10.4 音频主轨（不碰）

`setAmbient`/`setBGM`/`setTheme`/`playFoley`/`playStinger` 的签名与语义**一个都不改**（`docs/audio/00 §5` 冻结）。本批只在同文件**新增**语音三函数（§5）。

### 10.5 分页 vs 小天地（N1，入口历史说明）
N1 批次的历史方案曾把 Nook 入口放在 `RightSidebar` 角色行第 4 个按钮；UX 批次已裁决由 **App inline character rail** 拥有唯一入口。TTS 本批不拥有入口、不改 `RightSidebar`，也不得重新引入 `onOpenNook` callback。这里的 `RightSidebar` 只作为现状/历史引用。
...
**⚠️ 首句无声风险（现状证据，入口归属已迁移）**：`apps/web/src/lib/audio.ts` 的 `unlockOnFirstInteraction()`（`:926-934`）全仓零调用点；`unlock()` 的真实调用点只有 `Canvas.tsx:191` 与 `DiceRoller.tsx:219,265`。当前代码仍从 `RightSidebar` 触发角色遮罩（`App.tsx:585`），但 UX batch 的目标入口是 App inline character rail；实现时必须在最终遮罩打开链或遮罩首个 `pointerdown` 补一次 `void unlock()`，不得把 TTS 的音频解锁责任交给 `RightSidebar`。
...
| 文档 | 原说法 | 本批改为 | 依据 |
|---|---|---|---|
| `docs/doc-06 §3.2` | 「逐字流式」（整段） | 页内逐字 + **一行 = 一页 + 点击推进 + 页触发语音** | 用户 2026-09-13 需求（galgame 形态） |
| `docs/doc-06 §3.2` | 「玩家输入：对话框底部一行输入」（常驻） | **仅页翻尽后启用** | 同上 |
| `docs/doc-19 §2.3` | 「`[emo:]` 触发瞬时 stinger」 | **有语音时不播 stinger**（§6.5） | 人声与音效叠加冲突 |
| `docs/doc-07` / `doc-24` | TTS「P2 / 世界骨架后再评估」 | **本批落地**（需求已触发） | 用户 2026-09-13 决定 |
| `docs/doc-06 §3.2` | 「全脸眨眼/口型：先舍」 | **保持舍弃**（本批不做口型同步） | 范围控制；口型登记为后续 |
...
- **裁定：用 `locale` prop（UI 语言，`App.tsx:39-41` 的浏览器语言猜测）取 `GREETING_LINE[locale]`**，与 `03 §6.1` 实现一致。
- 理由：引导语是**给玩家看的 UI 开场**（「她在等你说话」），属 UI 文案层，跟玩家界面语言；而台词 TTS 的 `language` 跟世界内容语言（§15.4）。`06 §7` step 1 的「随 `world.json.locale`」**改为「随 UI locale」**。
- `GREETING_LINE` 的键集 = `locale` 的取值域 `{'en','ja'}`，超出回落 `'en'`。
