...
4. 解析 `voice`：请求 `voice` → 校验形状（裸名，`/^[A-Za-z0-9_-]+$/`）→ 否则回落 `AIRP_TTS_DEFAULT_VOICE`；无请求 voice → 默认。
...
| `pnpm check:skills` | 不变（本批不碰 skill） |
...
### 15.1 voice 正则：**接受含空格的厂商 ID 是错的 —— 保持 `/^[A-Za-z0-9_-]+$/`（04 冲突 1）**

04 主张放宽正则以容纳 `Eldric Sage`。**主 agent 实测（web 检索，2026-09-13）否决该前提**：

- 官方音色表里 `Eldric` 与 `Sage` 是**两个独立音色**（`Eldric` = "calm and wise" 男声；`Sage` = "calm, composed, and reassuring" 男声），**没有一个叫 `Eldric Sage` 的音色**。`Ono Anna`、`Radio Gol` 同理——`Radio Gol` 是单个 ID，但 `Eldric Sage` 是**两个 ID 被拼在一起**。
- 因此 04 §3.1 那份「逐字抄自官方表」的清单**含有不可核实的拼接成分**。**本批 MUST NOT 基于它放宽契约**。
- **裁决**：`00 §4.2` 步骤 4 的 `/^[A-Za-z0-9_-]+$/` **不变**。`archivist` 的 `voice` 取自**单 token、已由检索证实的**音色——用 **`Sage`**（"calm, composed, and reassuring"，气质贴合苍老守馆人）或 04 的保守备选 `Kai`。**MUST NOT 用 `Eldric Sage`**。

### 15.2 音色清单的信任等级：**降级为「候选」，落地前必须实测出声（04 冲突 5）**

- 检索证实 `Cherry` / `Serena` / `Ethan` / `Chelsie` / `Momo` / `Eldric` / `Sage` / `Radio Gol` **确实存在于 DashScope 生态**。故 04 的分配方向成立。
- **但 04 §3.1 那段「完整清单」未经厂商表逐行核对**（已证其中 `Eldric Sage` 为拼接）。**裁决：04 的全部 `voice` 取值标记为「候选值（`[未逐条核验]`）」，落地时 MUST 先跑 04 §9.1 的 curl 验证每个 ID 真能出声**，再定稿 README。用不存在的 ID = DashScope 400 = 该角色每一页无声。
- 已核实可用的**安全集合**（检索直接命中）：`Cherry`、`Serena`、`Ethan`、`Chelsie`、`Momo`、`Eldric`、`Sage`、`Radio Gol`。**分配优先从这 8 个里选**；选集合外的 ID 必须先实测。
