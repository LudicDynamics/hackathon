...
> 状态：设计批次共享契约（2026-09-14）。本文冻结本批「生成组件不重叠」残留治理与「整理」功能 Agent 的共同边界；各设计篇只能补充本模块细节，不得私改本文件的身份、真相源、事件和跨端接口形状。实现前须经过跨篇评审。
>
...
| 整理工具 | `arrange` 经 ActionService 写 canvas.db，现有 layout 支持 grid/circle/row；`link` 同样只写画布状态，均不落 history event，由工具结果映射 `canvas_patched` | `extensions/toolkit/arrange.ts:19-91`；`packages/shared/src/actions/canvas.ts:423-597`；`apps/server/src/engine/event-bridge.ts:291-320` | 整理 Agent 可复用动作内核，但必须扩充安全/真实尺寸语义或新增专用动作，禁止直接 SQL |
...
- 一个整理请求必须以一次可追踪的操作身份贯穿：接受、运行、每次动作、完成/失败/取消；重复请求不得并发覆盖同一层。是否使用已有 `arrangeCards` 还是新增批量动作，由设计篇提出并经评审冻结。
...
- 新 Agent 的业务来源统一称 `functional`，稳定 `agentId = 'canvas-arranger'`；一次点击使用独立 `turnId`（建议 `functional:canvas-arranger:<uuid>`，最终签名由设计篇核实），不能使用 `writer`/`character` 或把 session id 当 turn id。
- 整理写入采用**受控 functional Actor/Scope**，不复用 `engine`：现有 `engine` 是 trusted internal caller，且 `agentScope === 'engine'` 与 writer 等价放行 nook 变更（`packages/shared/src/actions/actor.ts:80-81,98-103`）。设计篇必须同步扩展 ActorType/Scope、reader/权限/事件与未知 role 的 fail-closed；不得让未知 `AIRP_AGENT_ROLE` 静默回退成 writer 而仍拥有整理能力。该项从“engine 或 functional 待选”升级为评审前必须完成的 functional 受控权限设计。
- Activity 帧的 `phase` 仍严格使用既有封闭集合 `started | completed | failed`（`apps/server/src/engine/agent-activity.ts:14`；前端 `apps/web/src/lib/agent-activity.ts:36`）。`accepted → processing → arranging → verifying → landed/completed` 只属于 D 的整理请求本地展示状态，必须有唯一映射，不得扩写 `agent_activity.phase` 或让 UI 猜测 spawn 即完成。
...
- 左侧按钮的玩家状态使用真实阶段：`accepted → processing → arranging → verifying → landed/completed`，并区分 `failed`、`cancelled`、**`conflict`**；`conflict` 是并发/版本或无法证明最新状态的独立本地终态，不得压扁成成功或普通取消。按钮不能显示「完成」而只代表 Agent 已被 spawn。
