...
采用**一次性生成批次的列流式排版（flow columns）**，替代原先以中心锚点为起点的 Ulam 螺旋（**已落地**：`packages/shared/src/layout/flow-columns.ts:152` 的 `flowColumns`）：
...
## 2. 已复核的现状与真实缺口（2026-09-14 回写）

> 本节原写于施工前，断言的是「缺口」；实现落地后逐条改为现状 + 残留边界。§0 的列流几何已取代服务端与幻影排座路径；仅 `seatNear`/`seatPresence` 与 `fitPhantom` 的局部兜底仍用螺旋（不在本契约的自动排版范围内）。

### 2.1 排版链路（已收敛到唯一 flowColumns）

- 新卡由 `apps/server/src/routes/world.ts` 的 `GET /layer` 读取；路由把当前页面的**完整 `items`** 传给 `store.seatUnplaced`（`apps/server/src/routes/world.ts:793-802` 构造 `seatFiles` → `:802` 调用），由 store 在事务内过滤无 row 者，再由 `reseatLayer` 处理占位漂移（`world.ts:808-810`）。
- `LocalWorldStore.seatUnplaced` 现在以 `flowColumns` 落位（`packages/shared/src/store/local-store.ts:889-918`，几何调用在 `:904`）；同批 `occupied.push`/`nextZ++` 不变量仍在。**旧螺旋候选已被替换**：`spiralCells` 只保留给 `seatNear`/`seatPresence` 的 near/presence 适配器（`local-store.ts:83,1492,1625`），不再参与自动排版。
- 共享占位契约与批内 occupied 追加仍在（`local-store.ts:903` 的 `layoutOccupied(...)`、`:904-918` 的 flow 落位；`docs/footprint/00 §3.2.1`）。
- `apps/web/src/lib/phantom-seat.ts:57-88` 的 `phantomSeatFor` **已改调共享 `flowColumns`**（`:75`），occupied = 当前层真实 rows + 同层全部未撤离幻影（Chalk + image，`:62-73`），**不再按 kind 过滤**。`fitPhantom`（`:17-26`）仍用 `seatSpiral`，但那是本地 DOM 兜底量测，不是幻影排座路径。
- `apps/web/src/components/canvas/Canvas.tsx:145-174` 只负责按真实 DOM 边界取景，不能再把它当第二个排版器；该 effect 明确禁止回写卡坐标。
- `apps/web/src/components/canvas/Canvas.tsx:313` 的 viewport dispatcher 现在对 `kind === 'chalk' && !allowChalkDrag` 早退；Chalk 专属锁由 `allowChalkDrag` prop 控制（`:47,125`），不再需要 `attention` 参与。
...
### 4.1 纯函数签名（已落地：`packages/shared/src/layout/flow-columns.ts`，由 `packages/shared/src/index.ts:19` 导出）
```ts
...
### 4.3 默认视觉参数（实现默认值；已在 `packages/shared/src/layout/flow-columns.ts:31-38` 的 `DEFAULT_CONFIG`）
