...
> 2026-09-13：玩家为 Holmes 本人，有 avatar，Watson 是陪伴者。高潮改为提交自己形成的推论与计划文章，再据此生成执行和结局；不固定一个万能最终动作。[六世界总览](六世界短闭环与AI生长验收.md)；设定见 [doc-24](doc-24-五世界可玩Demo体验设计.md)。
...
| 4–6 分钟 | 新建推论与计划两篇短文，向 Chalk 提交并确认 | Watson 引用我的推理；Writer 区分推论是否有依据、计划是否可执行 | 关键 Writer 介入；可代为整理口述，但玩家须核对 |
...
flowchart TD
    ...
    W --> D{推论有依据，计划可执行}
    D -->|依据不足| C
    D -->|可执行| E[玩家确认执行；Writer 写人物反应与物件安排]
...
- **已确认提交式高潮**：推论写“我认为发生了什么、依据是什么”，计划写“谁、何时、在哪、用什么做什么，以及预期风险”。日语版候选新文件为 `player/推論.md` 与 `player/作戦計画.md`；英语版为 `player/deduction.md` 与 `player/plan.md`。它们必须来自玩家的文字或经确认的口述整理，不是 Writer 自行写好再自动提交。材料交互遵守 [Chalk 约定](Chalk玩法形态与判定约定.md)。
