...
| 3–5 分钟 | 用 Chalk 写第一封短传真 | 一个后来时间的事实变化，角色回应这次改写 | Writer 改写相关内容，不为每封传真重生图片 |
| 5–7 分钟 | 读一次传真回执，再去对应时刻看同一个物件 | 回执并列写清旧事实、新事实与代价；不用记住两幅地图的差别 | Writer 更新普通 MD，短体验最多三封传真 |
...
- 世界规则沿用 `templates/divergence/skills/divergence-continuity/SKILL.md`：锚点物件允许跨时间携带；传真影响后来，不能顺便覆写已经展示的玩家行动记录。
...
- 传真长度仍有口径待统一：体验稿写“100 字”，现有英文 skill 写“100 words”。日语化时须明确计数与 UI 提示；本轮不假装已有统一机器校验。
...
当前记录：模板已有 `world/tokiwa-electrics/1994-11-02/`、`tonight/`、`thirty-years-later/` 与 `convergence/`，**现在实际是地点下分时间**，与目标不一致。需迁移目录、引用和 skill 后再验收；现有预写 convergence 不能充当本局实时改写的成功样本。本轮未调用模型、未验证三次传真与新现在的真实连通。
