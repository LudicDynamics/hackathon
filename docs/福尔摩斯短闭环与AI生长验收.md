...
当前体验版是 `templates/whitechapel-playtest/`；无后缀 `whitechapel` 保留为旧素材版。零级 `world/` → 一级 `world/london-map/`，五位 NPC 的注册、home、场景精灵、preset 和小天地已对齐。19项确定性／隔离HTTP检查通过；尚未用真实模型完整玩到结局，未生成新 CG，也未迁移现有存档。
