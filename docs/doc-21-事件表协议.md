...
一条事件落库**永远不触发 agent 的一轮**。doc-05 §5.1 的"不打断、不自动起 turn"是硬纪律；`routes/world.ts` 现有三处 `writer.prompt("[System notice: ...]")` 违反它（已登记于后端实现计划 §7 T4.2，本文不重复论证）。`use_item_on` 是否破例即时响应，是 B2 注入时机的问题，**与事件无关——不论是否破例，事件照落**。
