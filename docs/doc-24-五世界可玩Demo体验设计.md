...
| 时间线 | **写信 / 改写 / 对照** | 改变危险的取件旅程 | 玩家看见自己改写的未来 |
| 初雪 | **对话 / 承诺 / 实际赴约** | 一次关系变迁 | 两个 fallback 可收束，超出时由 RP 写出更多可能 |
...
| 时间线 | 遗憾的第一页：认领未接电话、发条青蛙与改变那一晚的目标 | **时间**：1994 / 1995今夜 / 2024是三个一级节点 | 先进入该时刻的**地点全览**，再进入常盘堂、巷口或其他场景；同一地点在不同时刻保持可对照的位置 | **已确认** |
...
| **两个 fallback + RP 开放结局** | 初雪 | 保留七海 / 澄两个收束方向与既有 CG，不预写最终正文 | 读取全层实际经历现场写短结局；复杂 RP 可超出两个方向 |
...
| 时间线 | 玩家自由写传真，Writer 把所得与失去物化 | 同一地点改变后的未来：陈设、人物处境与 Chalk 随改写改变 | 1 个重点对照段；中间不每次重生整图 |
| 初雪 | 对话更新记忆与约定；主动收束时依两个 fallback 或额外 RP 写短结局 | 主结尾复用既有 CG；避雪新环境只是额外可选 | 主线关系回合 + 结尾短文本；不强制实时生图 |
...
| **25 秒–3 分钟** | 进入第一个调查地点，得到首条事实 | 进入报社或停尸房，第一次出示证据 | 发第一封短传真，回来看到场景重排 | 第一次真实对话，留下回应与约定记录，不锁另一边 | 取得咒语碎片，书架路径改变 |
| **3–7 分钟** | 交叉验证三条短线索，城灯逐盏熄灭 | 证物与人物卡连成玩家自己的推理 | 再改写 1–2 次，代价变得可见 | 可继续访问两边、解释承诺，由角色回应实际 RP | 组合并命名一条咒语，施法改写空间 |
| **7–10 分钟** | 核验提交、查看风险、自主判定，分级回报驱动新路生成 | 提交推论与计划，确认执行后生成结果与物件 | 回执说明旧 / 新事实与代价，再去对应地点确认 | 选七海 / 澄 fallback 或自由 RP 收束，现场写短结局并匹配 CG | 魔法 + 道具 + 已确认代价形成不同短结局 |
| **10–30 分钟（可选）** | 进入新 Gate，只实例化新城镇的第一眼与一个可行动问题 | 只留下 New Case 钩子，不做第二案 | 只允许在改变后的未来多走一个场景 | 只留一条之后会被角色记住的事实 | 只进入天文台看见下一问题，不做下一章 |
...
> **已确认**：1994-12-31送修、1995-01-01事故后的夜晚、2024三十年后。玩家是同一个修理学徒在三个年龄的自己。完整因果与验收见 [时间线短闭环与AI生长验收](时间线短闭环与AI生长验收.md)。

### 5.1 主观短流程

零级先看修好的青蛙与无人签名的取件单，读到女孩在来取玩具的路上于东桥失足。拿起青蛙，再进入时间一级视图，每个时刻下仍先有地点总览。

1994店里有七岁的女孩、待修青蛙、次日十点取件的约定和过桥路线。1995与它共用一张店内背景，女孩缺席、蛙已修好、师傅沉默。FAX显示能够从元旦20:00送达同日08:00，早于08:30出发和09:00事故。

玩家可以写信请师傅联系家人、改为安全送货，核对正文与宛址后明确发送；也可以在1994通过RP改变取件约定。两种路线都判断是否真正改变危险旅程，不把FAX按钮当唯一钥匙。

### 5.2 生成回报与边界

- 2024始终一个入口、同一个店址。原线因师傅悲痛关店、无人接手而消失；救回女孩后可以仍营业，由长大的她接手。
- 一次有效干预即可结算；不要求三通传真，也不强加“救人必失店铺／FAX”的代价。
- Writer按真实原文／RP生成未来README、成年女孩实体、柜台青蛙和回执；她的短回应引用当年的约定。不是预写成功、不是完整三十年后日谈。
- 改写前记录实际旧事实；回访读既有结果，不重复发送。无效干预说明缺什么并允许修改，缺草稿就给写稿入口。
- 不增第四个必游的“新现在”；这个结果叫“改变后的未来”。图片后补，当前文字Mock，图像任务见 [素材工单](assets/分支线三时刻素材工单.md)。

---

## 6. 世界 D：初雪时间（First Snow）

> **已确认**：沿用 `firstsnow.json` 的电台导播玩家、七海、雪村澄与初雪期限。保留两个 fallback，但不强制二选一；只有实际行动与时间支持时才出现缺席痕迹，首次赴约不自动让另一人失约。
>
...
flowchart LR
    ...
    B --> C["1 级关系主视图<br/>选第一个目的地，不锁定另一边"]
    C --> D["真实对话 / 承诺 / 赴约<br/>更新角色记忆与两份约定"]
    D -->|继续访问、解释或改约| C
    D --> E["主动收束今晚<br/>七海 / 澄两个 fallback，或超出二者的自由 RP"]
    E --> F["汇总各层真实经历<br/>现场写结局，不预写第三条路线"]
    F --> G["匹配既有 CG<br/>3–6 句落在今晚，Demo 可结束"]
```
...
- 保留短体验与两个 fallback，可先后去两边、甚至都作出承诺；不另做日程引擎；
- 触发 ending 时匹配既有 CG 设计，不为演示临时扩充数量；核对真实资产路径；
- 根据此前各层已发生的 RP 现场写结局本体，不只读最后一幕；不做回忆册、检查点重玩或长后日谈；
- 所谓“变迁”是角色回应我真正做过的事，留下可见物件或关系变化；不强制把另一边写成冷杯与空椅。
...
### 6.4 已确认：两个 fallback + RP 现场结局

七海与澄各有一个基本收束选项，不要求玩家每次自由输入。没有实际互动时先给地点引导；有单边经历时能用对应 fallback 完成短体验；双边访问、双重承诺和额外 RP 可以产生第三种及更多可能，但不把“见过两人”直接映射到固定第三结局。

结局本体由作家在玩家主动收束时，根据各层实际对话、解释、到场与角色回应现场写成，不预写最终台词。既有 CG 服务这段内容，同一图能承载不同结局，不反过来强迫剧情进入两条路线。短结局 3–6 句，回访复用，不另开次日章节。实现与验收边界以 [初雪短闭环](初雪短闭环与AI生长验收.md) 为准；本轮只改体验版内容逻辑，不扩引擎。
...
| 3 | 时间线 | 已定案：怎样阻止取件路上的事故？ | FAX提前联系家人，或在昨日通过RP约定安全送货；不强加失去店铺 |
| 4 | 初雪 | 两个 fallback 如何承接简单经历，复杂 RP 如何现场收束？ | 已确认两个基础选项 + 更多可能，结局正文现场写；CG 复用 |
...
| 时间线 | 时间主视图、每个时刻下的地点锚点、传真时刻与可推导的事故因果 | 玩家写出的短信，以及同一地点在后续时间的可见改写 |
...
| 时间线 | 1994／1995共用背景；补成年女孩与2024同构图两状态，当前文字Mock |
| 初雪 | 使用既有 CG，按现场结局匹配；两个 fallback 不限制结局数量，不临时扩充 CG 数量 |
