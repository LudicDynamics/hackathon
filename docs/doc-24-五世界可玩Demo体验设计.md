...
| **7–10 分钟** | 说出“第二艘船”，`beyond` 出现 | 在不存在的街完成设局与短结算 | 选择一个新现在并看到所得 / 所失 | 只结算这一场雪中的关系变化 | 书架化为星图，天文台阶梯显现 |
| **10–30 分钟（可选）** | 只实例化税洞或追杀者落点之一 | 只留下 New Case 钩子，不做第二案 | 只允许在新现在多走一个场景 | 只留一条之后会被角色记住的事实 | 只进入天文台看见下一问题，不做下一章 |
...
flowchart LR
    ...
    F --> G["只打开一个后续 Gate<br/>Demo 结束"]
```

### 3.2 P0 边界

- 三个地点都保持“一条主线索 + 一次角色反应”，不扩写完整支线；
- `beyond` 只需出现并带一个依据玩家行动变化的追杀者落点；
- 不做营地结算、章节字幕、重置或完整雾外地图；
- 现有 `backgrounds/beyond.png` 足够承担高潮，不新增 CG。

### 3.3 待选择：玩家怎样亲手触发 punch line
...
```mermaid
flowchart LR
    A["雷雨夜<br/>传真机吐出未来来信"] --> B["拿起时间锚<br/>进入三个时间切片"]
    B --> C["发出第一封 ≤100 字传真"]
    C --> D["返回今晚<br/>同一场景肉眼可见地重排"]
...
|---|---|---|
| 雾坞镇 | 三条事实、三名角色纪律、灯光压力与一个 stub Gate | 调查顺序、告诉过谁，以及追杀者落点 |
| 福尔摩斯 | 固定真相、公平线索、人物知识边界与一个高潮场 | 玩家自己的证据链与最终设局动作 |
| 时间线 | 三个时间切片、传真限制与一组不可兼得代价 | 玩家写出的短信与被改写的新现在 |
| 初雪 | 一次直播、两份冲突约定与两个地点 | 玩家赋予物件的意义，以及赴约 / 缺席后的关系状态 |
| 魔法学院 | 三类碎片、代价规则与隐藏 Gate | 玩家命名的咒语实体与被改写的书架 |
