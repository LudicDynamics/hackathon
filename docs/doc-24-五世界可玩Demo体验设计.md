...
无限画布是共同底座，但每个世界用什么来组织一级节点，本身就是世界玩法的一部分。玩家在主视图上理解全局，点进 Gate 后才进入具体场景或操作面。

| 世界 | 一级主视图的组织轴 | 点进去之后 | 状态 |
|---|---|---|---|
| 雾坞镇 | **地点**：港城全览图，道路和 Gate 连接码头、工坊、灯塔与雾墙 | 进入某个地点的可交互场景；回到全览图时能看见灯火、道路和边界变化 | **已确认** |
| 福尔摩斯 | **地点**：伦敦案件地图，221B、第三案现场、报社、停尸房与“不存在的街”是一级节点 | 进入地点取证、见人或完成设局；案卷与人物卡是 221B 内的推理工具，不取代地点主视图 | **已确认** |
| 时间线 | **时间**：1994 / 今晚 / 三十年后 / 收敛点是一级节点 | 先进入该时刻的**地点全览**，再进入常盘堂、巷口或其他场景；同一地点在不同时刻保持可对照的位置 | **已确认** |
| 初雪 | **关系**：玩家在中央，两个角色、两项约定与共有物分布在两侧 | 点击某段关系上的约定或信物，才进入对应地点；返回时两边的距离、连线与缺席物已变化 | **体验方案** |
| 魔法学院 | **知识 / 法术**：活的咒语图谱，Verb / Subject / Cost 围绕未命名的法术节点 | 进入碎片或配方的操作面；施法后回到图书馆空间，看见被改写的书架与新地点 Gate | **体验方案** |
...
|---|---|---|
| 雾坞镇 | `wuwu.json` 已有港城图，下辖码头 / 工坊 / 灯塔 / `beyond` | 保留层级；把 `beyond` 从预写结果卡改为实时生成的新路与新城镇 Gate |
| 福尔摩斯 | `whitechapel.json` 的 `map` 当前是 221B 案卷桌 | 把一级 `map` 调整为伦敦案件地图；221B 与案卷桌成为其中一个地点及子层 |
| 时间线 | `divergence.json` 的 `map` 已按时间排列，但各时间 Gate 直接进入单一场景 | 保留时间一级视图；在每个时刻下加地点全览层，Demo 可只写常盘堂等必要地点，其余保留为 stub |
...
| **10–25 秒** | 收起铜徽，港城图亮起 | 拆信并拿起群青黄铜盖 | 拿起时间锚，三个时间 Folder 出现 | 收起一件关系物，第一处目的地亮起 | 令牌贴上索引，索引烧出玩家姓名 |
...
| **7–10 分钟** | 说出“第二艘船”，雾墙退开，实时生成通往新城镇的路与 Gate | 在不存在的街完成设局与短结算 | 选择一个新现在并看到所得 / 所失 | 只结算这一场雪中的关系变化 | 书架化为星图，天文台阶梯显现 |
...
```mermaid
flowchart LR
    A["事务所<br/>拿起调查人铜徽"] --> B["港城图<br/>选择无人船 / 工坊 / 灯塔"]
    B --> C["取得三条极短事实<br/>灯油 / 家纹销 / 熄灯十二分钟"]
    C --> D["向角色对证<br/>每靠近真相一盏灯熄灭"]
    D --> E["玩家亲自提出：还有第二艘船"]
    E --> F["Punch line<br/>雾中远船灯亮；身后一盏城灯灭"]
...
```mermaid
flowchart LR
    A["玩家说出第二艘船<br/>或完成最终触发动作"] --> B["Writer 收集本局事实<br/>路线 / 惊动的人 / 持有物"]
    B --> C["雾从港城图边缘退开<br/>半透明新路先出现"]
    C --> D["生成完成<br/>道路实体化，路尽头出现新城镇 Gate"]
    D --> E["可选进入 Gate<br/>scene-init 生成城镇的第一眼"]
```
...
```mermaid
flowchart LR
    A["221B<br/>Watson 把一项不能独自违背的承诺交给 Holmes"] --> B["读信<br/>Edith 曾求他保密；报社将在正午锁版"]
    B --> C["拿起沾群青的黄铜画筒盖<br/>第三案现场 Gate 亮起"]
    C --> D["伦敦案件地图<br/>进入报社 / 停尸房验证证据"]
    D --> E["玩家亲自推断<br/>凶手先看见的是图"]
    E --> F["不存在的街<br/>完成一个最终动作"]
    F --> G["短结算<br/>画第一次落空；Demo 结束"]
```
...
```mermaid
flowchart LR
    A["雷雨夜<br/>传真机吐出未来来信"] --> B["时间主视图<br/>选择 1994 / 今晚 / 三十年后"]
    B --> C["该时刻的地点全览<br/>进入一处场景并发出 ≤100 字传真"]
    C --> D["返回后续时间层<br/>同一地点肉眼可见地重排"]
    D --> E["再发 1–2 封<br/>救凉与保住常盘堂开始冲突"]
    E --> F["玩家选择愿意承担的新现在"]
    F --> G["一张结果场景<br/>所得与失去同时可见；Demo 结束"]
```
...
```mermaid
flowchart LR
    A["直播最后一分钟<br/>七海隔着玻璃叫玩家名字"] --> B["节目结束<br/>七海与澄的约定同时出现"]
    B --> C["玩家拿起一件关系物<br/>只选择一个目的地"]
    C --> D["赴约并交付<br/>角色记住玩家真的来了"]
    D --> E["初雪落下<br/>画面切到另一人留下的缺席痕迹"]
    E --> F["一条很短的结算 Chalk<br/>关系已经发生变化；Demo 结束"]
```
...
- 只做同一晚，不做连续数日、下一学期或巡演归来；
- 只做一次选择、一次赴约与一次缺席，不堆日程系统；
- 只需要两个很短的结果状态：去找七海，或陪澄去车站；
- 不做独处第三结局、回忆册、检查点重玩或完整后日谈；
...
| 选项 | 呈现 | 取舍 |
|---|---|---|
| **A. 同一张雪夜场景做两种人物 / 物件状态（推荐）** | 谁在伞下、谁留在空场景由状态切换 | 成本最低，最能证明“同一世界因行动改变” |
| B. 两张完整结局 CG | 七海线与澄线各一张 | 情绪更饱满，但资产成本翻倍 |
| C. 不出 CG，只用两个场景交叉切换 | 赴约处与缺席处各停一次镜头 | 更克制，但需要镜头和节奏足够成熟 |

本 Demo 在第一场雪的这一瞬间结束。玩家已经完成一次选择并看见关系变化，这就足够构成完整体验，不再追加后日谈。
...
>
> **主视图方案**：不以学院大地图开场，而以“活的咒语图谱”组织知识。Verb / Subject / Cost 是玩家可亲手拖合、命名的节点；场景 Gate 作为法术的锁与回报。当新法术成立，玩家回到图书馆看见空间被改写，天文台 Gate 才出现。

### 7.1 短 Demo 主观流程

```mermaid
flowchart LR
    A["晚钟后的图书馆<br/>书架自行移动"] --> B["黄铜令牌贴上自写索引<br/>出现玩家姓名"]
    B --> C["取得三类咒语碎片<br/>Verb / Subject / Cost"]
    C --> D["玩家组合并给咒语命名"]
    D --> E["对书架施法<br/>成功或失手都产生可用变化"]
    E --> F["Punch line<br/>图书馆化为星图，天文台阶梯出现"]
    F --> G["只踏上第一级台阶<br/>Demo 结束"]
```

### 7.2 P0 边界

- 首个 Demo 只提供一组三选一组合，不搭完整法术系统；
- 只生成一张玩家命名的 spell Markdown，证明玩家能创造世界实体；
- 失败只改变代价或书架形态，不阻止天文台揭示；
...
|---|---|---|---|
| 1 | 雾坞镇 | 玩家用哪个动作亲手触发“第二艘船”揭示？ | A：转动灯塔透镜 |
| 2 | 福尔摩斯 | 已确认 Watson 对 Edith 的承诺、正午锁版、黄铜盖与第三案现场 Gate；下一步再选高潮动作与结算深度 | 开场 A 已确认；其余待讨论 |
| 3 | 时间线 | Demo 最终让玩家在什么两件不可兼得的东西中选择？ | A：凉活着 vs 常盘堂与传真仍在 |
| 4 | 初雪 | 哪件物品承载关系？结尾用状态变化还是完整 CG？ | A：节目台本；同场景双状态 |
| 5 | 魔法学院 | 玩家第一条咒语付出什么代价？ | A：一段具体记忆 |
...
| 初雪 | 暂不生产三张结局 CG；优先讨论“同一雪夜场景双状态”是否足够 |
