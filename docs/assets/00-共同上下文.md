...
1. **本文冻结的形状**（路径、字段名、命名、解析链；媒体尺寸只作建议）；
...
- **微动立绘（透明 webm）** → 走**角色小天地的角色自身媒体投影**（Nook Canvas 外的 direct DOM，不是画布卡片）；
- **背景微动（不透明 webm）** → 走**场景 README `bgVideo:`**（niko 批已完成，本批不动）。

一句话口径：**遮罩要「换表情」，所以用图；小天地要「像活的」，所以用视频。** 二者应尽量保持同族视觉，但具体分辨率与长宽比是生产建议，不是发布或运行时的硬门槛。
...
| # | 需求（用户原话） | 形态 | 落点 | 状态 |
|---|---|---|---|---|
| 1 | 背景图换 webm 微动视频 | 建议 1280×720、VP9、6s 循环 | `assets/motion/seedance/backgrounds/*.webm` | ✅ niko 批已完成 |
| 2 | 角色图片补充 6 种情绪拆分 | 6 张透明 PNG/webp；建议接近 9:16 | `assets/characters/<id>/<emo>.webp` | ❌ 待生产 |
| 3 | 角色立绘转微动视频（透明底） | 透明 VP9 webm；建议 360×640 | `assets/motion/seedance/characters/<id>-transparent.webm` | ⚠️ 部分 |
| 4 | BGM 自己生成氛围音（建议） | 世界主题曲 mp3 | `assets/audio/themes/*.mp3` | ✅ 已有；可选重生成 |

**范围边界**：门禁扫描所有已存在的 `templates/*` 发布位；当前生产源仍按实际角色和世界逐步补齐。没有情绪目录的模板不因未生产情绪图而失败。
...
| 尺寸 | **建议**宽高比接近 9:16（banana portrait 常见输出 768×1376 系）；发布门禁不以具体尺寸或比例失败 |
| 透明 | **真透明（alpha）**，绿幕抠像产出，与微动 webm 同源同族 |
| 编码 | 发布位 webp（q82，与 `sync-template-assets.mjs` 既有口径一致） |

尺寸是视觉建议，不是资源形状契约。不同来源或平台输出的有效长宽比可以直接发布，由组件的 `object-fit` 和运行时布局适配。
...
| 尺寸 | **建议** 360×640；不作为门禁硬条件 |
| 编码 | VP9 `yuva420p` + `alpha_mode=1`，`--pingpong` 无缝循环，无音轨 |
| poster | 从 webm 自身首帧导出 PNG（`ffmpeg -c:v libvpx-vp9 -frames:v 1`），逐像素同源 |

### 3.3 背景微动（既有，只读）

`templates/<world>/assets/motion/seedance/backgrounds/*.webm`，建议 1280×720 VP9 6s 循环。**本批不重做**，只在文档中登记其已完成。
...
`/api/characters` 返回的角色 `avatarVideo` / `avatar` 是 Nook 角色自身媒体的唯一输入：
Nook 在 Canvas 外挂载 direct media DOM；**不创建** `templates/<world>/characters/<id>/portrait.md`
或 `spirit.md`，也不把角色媒体写入 `cards` / `presence`。`portrait` kind 仅供普通画像/
陈设 `LayerItem` 使用（详见 `docs/nook/00 §3.5`）。
---
...
```bash
# 0. 一键验收门禁（只读）：6/6 齐备 + 真透明 + webm 真 alpha + manifest SHA + 双模板字节一致
pnpm check:emotions            # = node tools/verify-emotions.mjs [--world <template>] [--all]
...
2. 角色静态图与视频**建议**保持接近的视觉比例，以减少回退时的跳变；具体分辨率、长宽比不得作为发布门禁硬失败条件。
3. **MUST NOT** 用 ffmpeg 内置 vp9 解码器验 alpha（静默丢 alpha，见 `motion-portrait/SKILL.md` 坑一）；MUST 显式 `-c:v libvpx-vp9`。
4. 生产时**不要假设** 360p 视频档可用；当前上游建议使用 720p，并以实际 provider 能力为准。发布资产的分辨率与长宽比不锁定。
...
| 事实 | 证据 |
|---|---|
| 背景 webm 已完成 | `templates/*/assets/motion/seedance/backgrounds/` 已有多世界动态背景；1280×720 是推荐值，不是门禁条件 |
| 角色动态片 | 各模板按实际角色配置存在不同覆盖率；门禁扫描所有已存在的 `*-transparent.webm`，不要求每个角色都必须有视频 |
| 6 情绪差分 | 对所有模板中已出现的情绪目录执行 6/6、真透明与清单校验；没有情绪目录的模板不报缺失 |
