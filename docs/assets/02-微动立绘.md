...
**微动立绘 = 把一张绿幕情绪图（`normal-green`）喂给 i2v 生成 6 秒微动片，抠成 360 宽的透明 VP9 webm，再把该 webm 的自身首帧导出成 poster，最后把 `video:`/`poster:` 两条相对世界根路径写进 `templates/<world>/characters/<id>/portrait.md`——小天地据此把「一张会动的贴纸」摆在画布上。**

一句话口径（assets 00 §1）：**遮罩要「换表情」，所以用图；小天地要「像活的」，所以用视频。** 两者**同源同族**：同一张 `normal-green` 出的静态 PNG（`01`）与动态 webm（本文）都是绿幕抠像、同为真 alpha、同为 9:16 家族（图 768×1376，视频 360×640），因此视频坏了回退到静态帧时**画面不跳变**。

```mermaid
graph LR
  G["ID/variants/normal-green.jpg<br/>(01 的产物, 零额度)"]
  G -->|"flow-gen video<br/>omni-1.1-flash i2v 720p 6s<br/>10 credits per clip"| M["ID-green.mp4<br/>车间 720x1280 绿幕"]
  M -->|"motion-clip<br/>chromakey + despill<br/>scale 360 / pingpong"| W["ID-transparent.webm<br/>发布位 360x640 alpha"]
...
| 面 | 消费者 | 形态 | 理由与出处 |
|---|---|---|---|
| 遮罩（直聊对话框） | `CharacterModal` 的 `.portrait-slot` | **静态 6 情绪图** | `[emo: tag]` 要求**即时换脸**：一次切换只需一次 `src` 变化，而视频换「表情」要么换整条 webm（重解码、必卡）要么做多段时间轴同步（T3.2 的「零加载闪烁、一次请求」目标直接破功）。出处：`assets/skills/motion-portrait/SKILL.md:103-104` 逐字「**直聊遮罩：不用视频**——那里要 6 情绪差分快速切换，静态立绘 + CSS 呼吸更合适」；`docs/前端改造计划.md:363-367`（T3.2 的表情表机制）。 |
...
**3. i2v 参数：720p/6s/portrait，`--similarity 0.06`。** 分辨率只能 720：`360p` 生成档上游已关闭（assets 00 §4.2 实测，`VIDEO_RESOLUTION_360P` 枚举合法但档位层拒），**MUST 720p**。`--similarity` 从默认 0.12 压到 **0.06**：`assets/skills/motion-portrait/SKILL.md:83` 逐字「阈值一大，角色本身的暗部（深蓝毛衣、黑发）也会被一起吃掉」；本批实测 `nanami-transparent.webm` 的绿残留 **0.00%**（不透明像素里无绿主导）——**降阈值零代价**。
*漏了会怎样*：用 360p → 上游 `INVALID_ARGUMENT`（`flow-gen` 报错退出）；用默认 0.12 → 角色暗部出洞（肩线碎裂）；不写 `--pingpong` → 首尾帧对不上（`assets/skills/motion-portrait/SKILL.md:63` 逐字「实测 MSE ≈ 500，直接 `loop` 播会"跳一下"」）→ 循环时「跳一下」。
