...
>
> **落地状态（2026-09-13 实装后）**：`tools/verify-emotions.mjs` 已落地并接入 `pnpm check:emotions`。现在它扫描所有含 `world.json` 的 `templates/*`：已有情绪目录必须 6/6、真透明并有溯源账；所有已存在的透明角色视频检查真实 alpha；尺寸与长宽比只输出 advisory。落账使用 `templates/<world>/assets/character-media.json`（`tools/record-emotion-assets.mjs`），不并入 sync 工具的两份 manifest。
...
| `verify-emotions.mjs` | **已落地**，扫描所有 `templates/*` 的角色媒体 | — |

**门禁脚本**：

```text
tools/verify-emotions.mjs —— 6/6（已存在的情绪集）/ 真透明 / 真 alpha / character-media SHA / 双模板字节一致 / 视频优先接线
```

尺寸和比例不是失败条件；生产时仍可通过 advisory 观察推荐值是否被上游静默降级。
...
**V1. 回写与代码同 commit。**
`AGENTS.md:315-317` 逐字要求「改了架构就必须在**同一个 commit** 里改文档」。**漏了会怎样**：下一个 agent 读到 `docs/ui/前端改造计划.md:643` 的「剩余：6 情绪精灵表（T3.2）」会照精灵表再实现一遍——而契约 §3.1 冻结的是**六张独立 webp**（⑪-C2 是同款已发生的例子）。
...
| **W10** | 发布位新增 `assets/characters/<id>/<emo>.webp` | `docs/worlds/模板资源对齐清单.md:9-23`（本轮已落实表，`:16` firstsnow 行后） | 表下加两条：`- whitechapel 的 watson/edith/wayne/blackburn/tom 各 6 张表情差分已落发布位（assets/characters/<id>/<emo>.webp）`；`- firstsnow 的 nanami/sumi-yukimura 同上；first-snow-jp 为逐字节副本` | 该文件的用途就是登记"素材与引用完整性"（`:32` 逐字区分"素材完整 ≠ 可玩性已验收"） |
| **W11** | `porter` 本批跳过 | `docs/worlds/模板资源对齐清单.md:31` | **不改**：该行逐字「雾都 Porter 暂无正式立绘；不借其他角色图片填充」正是契约 §8 反模式 5 的依据。只在此**确认不改** | 契约 §8.5 |
...
### 4.2 B 组 —— 机检脚本（`tools/verify-emotions.mjs`）

**签名**：

```text
node tools/verify-emotions.mjs [--world <template>] [--all] [--json]
```

脚本动态发现所有含 `world.json` 的 `templates/*`，不再复制一份固定世界表：

| 查 | 行为 |
|---|---|
| A | 对已经出现情绪文件的角色目录检查 6/6 webp；没有情绪目录的模板不报缺失 |
| B | 用 `dwebp` + RGBA 解码检查 webp 真透明 |
| C | 记录 webp 和角色 webm 的尺寸/比例 advisory，不参与失败判定 |
| D | 用 `ffmpeg -c:v libvpx-vp9` 检查透明角色视频真 alpha，并校验存在的 `character-media.json` 摘要；有情绪目录但缺账本则失败 |
| E | 检查 firstsnow 与 first-snow-jp 的发布媒体逐文件字节一致 |
| F | 若 frontmatter 只引用图片，而同名透明视频存在，则失败并要求补 `avatarVideo`；背景同理要求 `bgVideo` |

分辨率与长宽比只写入 `advisories`，不改变退出码。`--json` 返回 `worlds`、`findings`、`advisories` 和 `clean`。

关键内部函数：

| 名 | 作用 |
|---|---|
| `templateDirs()` | 发现所有模板目录 |
| `emotionCharacters(template)` | 从已出现的情绪目录与 `character-media.json` 推导检查对象 |
| `decodeAlpha(file, kind)` | webp 走 `dwebp`，webm 显式走 `libvpx-vp9` |
| `checkVideoPreference(template, findings)` | 防止同名视频存在时仍只引用图片 |
| `checkDoubleWorld(findings)` | 校验 firstsnow 双入口的媒体字节一致 |
`decodeAlpha` 不从生产脚本导出：webp 使用 `dwebp` 解码以避开本机 ffmpeg 的 webp 兼容性问题，webm 显式使用 `libvpx-vp9`，两条管道都在门禁脚本内独立执行。
...
| D5 | `docs/ui/前端改造计划.md:363-367` 仍主张**精灵表** | `:363-367`、`:618`、`:643` | 改为六张独立 webp（W7–W9） |
...
`docs/ui/前端改造计划.md:363-367`（T3.2 正文逐字「走精灵表，不走 6 个独立 PNG」）、`:618`（素材清单「精灵表而非 12 个 PNG」）、`:643`（P3 待办「剩余：6 情绪精灵表（T3.2）」）。`index.css:403-405` 的注释也逐字指向精灵表（`background-position sprite switching on .portrait-emo`）。
