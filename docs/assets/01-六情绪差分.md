...
**把「六情绪」从 CSS 滤镜的幻觉，换成六个真透明的、同族同尺寸的图片文件，并把「哪一个文件」的唯一决定权交给服务端探测**：生产侧一条零额度图片链（`base.png` → img2img 绿幕 → chromakey 抠像 → `cwebp -q 82`），消费侧一条单向回退链（有全套 `emotions` → `<img>` 按 emo 切图；缺一张或旧世界无素材 → 原样回退 `MotionPortrait(video, poster)`）。
...
1. **`templates/<world>/assets/characters/<id>/<emo>.webp` 是新增的发布位子目录**（assets 00 §3.1 冻结）。`04` MUST 把 6×N 条新增媒体按 `{ source, target, sourceSha256, targetSha256 }` 记入 `templates/<world>/assets/source-manifest.json`（现有形状：`{ sourceRepository, encoding, assets: [...] }`，实测 `templates/whitechapel/assets/source-manifest.json` 当前 12 条，`encoding: 'webp-q82'`）。
...
`templates/whitechapel/world.json:68-73` 的 porter 只有 `id` / `home` / `role` / `description`，**无 `avatar` / `avatarVideo`**；其余五个都有 `avatar`。`docs/模板资源对齐清单.md:31` 逐字：「雾都 Porter 暂无正式立绘；**不借其他角色图片填充**。」
...
**U4 · 生成后的尺寸校验要不要加？**
脚本不校验输出尺寸（§7 表末行）。若上游把 `portrait` 静默降级成 `landscape`（`flow-gen.mjs:246-265` 只**打印**警告，不阻断），我们会得到一张 1376×768 的横图，此后 `cwebp` 照常编码，`index.css:420` 的 `object-fit: contain` 会把它缩成一个小横条——**没有任何环节报错**。
**待拍板**：建议在 `toWebp` 前加一条 `ffprobe` 尺寸断言（≥1.70 且 ≤1.85 的宽高比）并在不符时 `exit(1)`。这是**新增功能**，超出「接线 + 生产」的最小范围，故列为待拍板而非本片默认动作。
...
**U6 · firstsnow 与 first-snow-jp 的情绪图同步方式**
assets 00 §6.2 要求「两模板同名同字节」。但 `gen-emotions.mjs` 目前**只写 `templates/<worldName>/`**（`:170` `path.join(ROOT, 'templates', worldName, …)`），`main` 只接受 `--world whitechapel|firstsnow`（`:42-89` 的 `CHARACTERS` keys），**没有 `first-snow-jp` 条目**。
**待拍板**：同步由谁做——（a）`gen-emotions.mjs` 加 `first-snow-jp` 并生成两次；（b）`03` 在双模板同步任务里 `cp`；（c）`04` 的机检脚本负责。本文倾向 **(b)**（对称于 `tools/sync-seedance.mjs:20` 的 `mediaWorlds['first-snow-jp'] = mediaWorlds.firstsnow;` 既有做法——一份配置两处落位）。**本片不实现同步**，只登记接口：生成器产 `templates/firstsnow/…`，`03`/`04` 负责 `first-snow-jp` 的字节副本 + SHA 断言。
...
| 上游文档明确不补 | `docs/模板资源对齐清单.md:31` 逐字「雾都 Porter 暂无正式立绘；不借其他角色图片填充。」 |
