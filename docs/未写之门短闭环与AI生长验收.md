...
>
> **前置口径**：本文描述的节拍要求「交互之后立刻看见后果」，**这依赖把该世界的 `autoWrite` 打开**（`scenes` 或 `scenes-and-choices`）。默认 `off` 下，动作只落事件、作家等玩家下一轮输入才回应——**默认档下本文的时间线不成立**。逐交互档位见 `docs/settings/00-共同上下文.md §1bis`。
>
