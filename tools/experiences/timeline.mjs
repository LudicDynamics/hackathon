...
p.scene('world', '導入 · 一九九五年一月一日、午後八時', 'あなたは常盤電器で蛙を修理した若い見習い。昨日リョウと約束した「明日の朝十時」は、もう過ぎた。蛙は跳ねる。少女は帰らない。事故の知らせには、今朝九時、東橋で店へ向かう少女が転落したとある。', ['手紙を読み、ぜんまいの蛙を持つ', '事故の知らせを読む'], { bg: shop, intent: '取ると言われたら world/clockwork-frog.md を player/clockwork-frog.md へ move。手紙も希望された場合のみ player/undelivered-letter.md へ move。蛙だけで時間図へ入れる。事故の知らせは読むだけでよい。救済を先取りしない。' });
...
for (const [id, title, bg, body, choices, intent] of [
  ...
  ['thirty-years-later', '二〇二四年十二月三十一日 · 三十年後', 'assets/scenes/thirty-years-later.webp',
    '年を取ったあなたが店の跡に立つ。少女を失った師匠は店を閉め、引き継ぐ人のいない建物は後に取り壊された。ここはまだ変えていない未来。背景は既存素材の Mock。店の有無は記録で確かめられる。',
...
// Reuse the visually reviewed child portrait; the adult remains a text mock.
...
1994と1995の bg は同じ ${shop}。年月日、人物、蛙、署名欄で違いを見せる。2024の二状態も同構図が目標。画像は worldlines-assets で後から制作する。この Mock では generate_image を呼ばず、架空の画像パスを書かない。文字結果を先に完成し、画像がなくても読める。1–2分は生成体験予算で速度保証ではない。`);
export default p;
