...
test('failure has real bounded loss, no oil cost for 4–6, and concrete recovery choices', () => {
  const failure = section('3. 2–3', '5. 7–10');
  assert.match(failure, /player\/lamp-oil.md[\s\S]*空瓶/);
  assert.match(failure, /4–6 なら油を減らさず/);
...
test('success preview cannot generate a full reveal or issue the great-success reward', () => {
  const preview = section('5. 7–10', '6. 成功した');
...
test('only a success continuation click reveals the scene; great rewards remain distinct', () => {
  ...
  assert.match(reveal, /11–12 の時だけ player\/introduction-letter.md/);
  assert.match(reveal, /7–10 にこの追加報酬を渡さない/);
