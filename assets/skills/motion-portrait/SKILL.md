...
### `--despill-expand` 不是调参旋钮（2026-09-15 实测，默认已改为 0）

脚本曾默认 `--despill-expand 0.4`（偏离 ffmpeg `despill` 自身的默认 0）。**那是有害的。**

在三块互不相干的绿幕（`fpal/mingyue`、`whitechapel/tom` 视频、`tom/variants/smile` 图）上扫
`mix × expand`，量两件互为代价的事：半透明边缘的绿溢出，以及主体（衬衫/开衫）的颜色均值：

| 绿幕 | 无 despill | `expand=0` | `expand=0.4` |
|---|---|---|---|
| mingyue | 边缘绿 **18.6** | 边缘绿 **0.00**，主体 G=166.8 | 绿 0.00，主体 G=**152.3**（−14.5） |
| tom | 边缘绿 **82.5** | 边缘绿 **0.00**，主体 G=70.2 | 绿 0.00，主体 G=**63.0**（−7.2） |
| tom-smile | 边缘绿 **100.3** | 边缘绿 **0.00**，主体 G=55.2 | 绿 0.00，主体 G=**52.4**（−2.8） |

**结论**：`mix` 单独就已把边缘绿清到 0；`expand` 再调大**一个像素的绿都不多去**，
只是持续压低主体的绿色通道。实况：明月的米白衬衫在 `expand=0.4` 下被压成**粉色**，
而 R/B 通道纹丝不动——这正是"只吃绿色"的签名。

**判据**：主体颜色偏移看**绿通道单独变化**（R/B 同步不变 ⇒ 是 despill 的锅，不是整体色调问题）。
不要靠肉眼对着叠了纸色的预览判断——暖纸色会让白衬衫**看起来**发粉，那是合成错觉，不是抠像故障。

只在**某个具体片子**仍见绿边时才手动加大 `--despill-expand`，且改完必须重抽一帧比对主体色。
