...
export function buildItemActionPrompt(text: string, items: DraftItem[]): string {
  ...
  return `${text}\n\nReferenced carried items (exact paths): ${JSON.stringify(paths)}. Read every referenced item and the current scene README before resolving the player's final edited request. Use read for the full README/frontmatter, including intent; look_at is only a player-facing summary. Reuse records already read this turn instead of repeatedly rereading unchanged files. These references identify objects, not automatic permission to consume, combine, move them, or bypass scene requirements. If the intended target or combination is unclear, ask in Chalk. For a valid action, use the existing action tools and persist required item/state changes, then provide the result and next available action in Chalk. Resolve only this action, then wait for my next input.`;
}
