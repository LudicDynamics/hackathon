...
export function TtsSettings() {
  ...
  return <>
    ...
    {open && createPortal(<div className="settings-backdrop" onClick={() => setOpen(false)}>
      <section className="settings-panel" role="dialog" aria-modal="true" aria-label="Voice settings" onClick={e => e.stopPropagation()} onKeyDown={e => { if (e.key === 'Escape') { e.stopPropagation(); setOpen(false); } }}>
        <header className="settings-panel__header"><h1>{t('Settings')}</h1><button onClick={() => setOpen(false)} aria-label="Close voice settings" autoFocus>×</button></header>
        <h2>{t('Audio volume')}</h2>
        {(['voice', 'music'] as const).map(channel => <label className="settings-volume" key={channel}>
          <span>{channel === 'voice' ? t('TTS voice') : t('BGM / theme music')}<output>{Math.round(volumes[channel] * 100)}%</output></span>
          <input type="range" min="0" max="100" step="1" aria-label={channel === 'voice' ? t('TTS voice') : t('BGM / theme music')} value={Math.round(volumes[channel] * 100)} onChange={event => changeVolume(channel, Number(event.target.value) / 100)} />
        </label>)}
