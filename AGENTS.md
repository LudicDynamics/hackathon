...
语音设置：`GET /api/tts/config` 仅暴露配置就绪状态、线上模型和默认音色；前端“声音与设置”控制本浏览器语音开关、语音与背景音乐音量，未配置时停止合成请求并单次提醒。角色语音区可配置七海本机服务基址、声音 ID、超时和可选密钥，经连接设置路由保存后下次合成立即生效，失败仍警告并回落线上；面板文案支持英中日。详情见 `docs/tts/TTS设置面板.md`。
